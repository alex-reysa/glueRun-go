...
- `engine/ctx-planner-session.sh`: canonical per-node planner session-meta path
  `.singular-state/sessions/planner/<node>.json` (state dir, NOT docs/ — session
  ids are runtime state, not repo truth), reusing the existing
  `singular.orchestration.session-meta.v0` shape with `role: "planner"` and a new
  optional `node` field.
- Hook `generate-tasks.sh` (and the staged path in `l1-plan-node.sh`) to pass
  ...
  batch (reuse `singular_session_meta_finalize`; lineage anchor = the target
  branch head at planning time). Behind `SINGULAR_PLANNER_SESSION=0`.
...
- `engine/ctx-planner-resume.sh`: `singular_planner_resume_decide` — the
  ...
  - SHA ALIGNMENT (binding, added after TASK-0008 review): the integrated
    ...
    `singular_ctx_planner_session_finalize` (re-own
  ...
  - ADD: **session lease** — `.singular-state/sessions/planner/<node>.lease`
...
- `generate-tasks.sh` consults it only when `SINGULAR_PLANNER_SESSION=1`;
