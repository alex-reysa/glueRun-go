...
#   gluerun_graph_canonicalize   projected JSONL on stdin -> deduped + id-sorted
#   gluerun_graph_write_corpus   <graphdir> <nodesIn> <edgesIn> -> nodes.jsonl + edges.jsonl
#
...
# ${GLUERUN_CTX_GRAPH_DIR:-.gluerun-state/graph}, round-trips lines from the real
# gluerun_graph_emit_node/emit_edge primitives back through the schema, preserves
...
for fn in gluerun_graph_canonicalize gluerun_graph_write_corpus; do
...
nA="$(gluerun_graph_emit_node finding    'finding:F1' 'docs/critique/F1.md' 'body A' 'a finding')"
nB="$(gluerun_graph_emit_node task       'task:T1'    '.gluerun-state/tasks/T1' 'body B' 'task one')"
nC="$(gluerun_graph_emit_node commit     'commit:C1'  'refs/commit'         'body C')"
nD="$(gluerun_graph_emit_node gate-result 'gate:G1'   '.gluerun-state/gates/G1.json' 'body D' 'gate' '{"verdict":"pass"}')"
fromT="$(gluerun_graph_node_id 'task:T1')"
toA="$(gluerun_graph_node_id 'attempt:T1#1')"
eX="$(gluerun_graph_emit_edge implements "$fromT" "$toA" '.gluerun-state/events/log.jsonl' 'edge body X')"
eY="$(gluerun_graph_emit_edge verifies   "$fromT" "$toA" '.gluerun-state/events/log.jsonl' 'edge body Y')"

# --- Slice 1: canonicalizer --------------------------------------------------

# Dedup: an identical line repeated collapses to a single output line.
dup_out="$(printf '%s\n%s\n%s\n' "$nA" "$nB" "$nA" | gluerun_graph_canonicalize)"
[[ "$(printf '%s\n' "$dup_out" | wc -l | tr -d ' ')" == "2" ]] \
...
perm1="$(printf '%s\n%s\n%s\n%s\n' "$nA" "$nB" "$nC" "$nD" | gluerun_graph_canonicalize)"
perm2="$(printf '%s\n%s\n%s\n%s\n' "$nD" "$nC" "$nB" "$nA" | gluerun_graph_canonicalize)"
perm3="$(printf '%s\n%s\n%s\n%s\n' "$nB" "$nD" "$nA" "$nC" | gluerun_graph_canonicalize)"
...
gluerun_graph_write_corpus "$gdir" "$nodes_in" "$edges_in" \
...
cmp -s "$gdir/nodes.jsonl" <(gluerun_graph_canonicalize < "$nodes_in") \
  || fail "nodes.jsonl is not the canonicalized node input"
cmp -s "$gdir/edges.jsonl" <(gluerun_graph_canonicalize < "$edges_in") \
...
gluerun_graph_write_corpus "$gdir" "$nodes_in" "$edges_in" || fail "second write_corpus failed"
...
rm -rf "$gdir"
gluerun_graph_write_corpus "$gdir" "$nodes_in" "$edges_in" || fail "rebuild write_corpus failed"
...
gluerun_graph_write_corpus "$gdir" "$nodes_in" "$edges_in" || fail "rewrite over stale corpus failed"
...
(
  cd "$def_root"
  export GLUERUN_CTX_GRAPH_DIR="$def_root/custom-graph"
  gluerun_graph_write_corpus "" "$nodes_in" "$edges_in"
) || fail "write_corpus with default graphdir failed"
[[ -f "$def_root/custom-graph/nodes.jsonl" ]] \
  || fail "write_corpus did not honor GLUERUN_CTX_GRAPH_DIR default"
...
target="$(mktemp -d)/g"
( cd "$work" && gluerun_graph_write_corpus "$target" "$nodes_in" "$edges_in" ) \
