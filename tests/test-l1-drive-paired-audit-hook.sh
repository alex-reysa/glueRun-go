...
# gluerun_ctx_paired_audit_record from the integrated engine/ctx-paired-audit.sh.
#
#   (a) default-OFF (GLUERUN_PAIRED_AUDIT_PCT unset AND =0): the accepted path is
...
#       inbox packet at $GLUERUN_INBOX_DIR/$run_id.json, and the l1.task_accepted
...
workroot="$(mktemp -d "${TMPDIR:-/tmp}/gluerun-paired-hook.XXXXXX")"
...
reset_state
out="$(run_drive GLUERUN_PAIRED_AUDIT_PCT=0 2>&1)" || { echo "$out" | tail -20; fail "OFF (=0): drive failed"; }
...
# ---------------------------------------------------------------------------
reset_state
out="$(run_drive GLUERUN_PAIRED_AUDIT_PCT=100 2>&1)" || { echo "$out" | tail -20; fail "PCT=100: drive failed"; }
...
reset_state
ec=0
out="$(run_drive GLUERUN_PAIRED_AUDIT_PCT=100 FAIL_PAIRED=1 2>&1)" || ec=$?
...
reset_state
ec=0
out="$(run_drive GLUERUN_PAIRED_AUDIT_PCT=100 MOCK_AUDIT_VERDICT=needs-fix 2>&1)" || ec=$?
