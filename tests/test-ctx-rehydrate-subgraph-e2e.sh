...
# gluerun_ctx_rehydrate_subgraph_select), the node->source resolver (TASK-0101
# gluerun_ctx_rehydrate_subgraph_sources), the order-preserving assembler
# (TASK-0102 gluerun_ctx_rehydrate_subgraph_assemble), the A/B arm decider
# (TASK-0104 gluerun_ctx_rehydrate_subgraph_arm_mode), the ctx-route.sh manifest
# selector (TASK-0105 gluerun_ctx_route_subgraph_manifest), and the shared inject/
# record selector (TASK-0106 gluerun_ctx_route_subgraph_render). Each is covered
...
# composed path — gluerun_ctx_rehydrate_subgraph_assemble and the WIRED selectors
...
#   * Caps unchanged: section bodies capped at GLUERUN_CONTEXT_SECTION_MAX_CHARS
...
#     strategy stays tainted per gluerun_ctx_route_strategy_tainted.
#
...
# Hermetic guard: scrub inherited GLUERUN_* so a leaked knob can't poison the run.
...
for fn in gluerun_ctx_rehydrate_subgraph_select gluerun_ctx_rehydrate_subgraph_sources \
          gluerun_ctx_rehydrate_subgraph_packet gluerun_ctx_rehydrate_subgraph_manifest \
          gluerun_ctx_rehydrate_subgraph_assemble gluerun_ctx_rehydrate_subgraph_arm_mode \
          gluerun_ctx_route_subgraph_render gluerun_ctx_route_subgraph_manifest \
          gluerun_graph_identity gluerun_graph_node_id gluerun_graph_emit_node \
          gluerun_graph_emit_edge gluerun_graph_write_corpus gluerun_ctx_ab_arm_for \
          gluerun_ctx_rehydrate_sources gluerun_ctx_rehydrate_manifest \
          gluerun_ctx_route_strategy_tainted; do
...
section_body() {
  GLUERUN_SB_ID="$1" python3 -c '
import os, sys
want = os.environ["GLUERUN_SB_ID"]
...
# provenance.sourcePath. The task node identity is gluerun_graph_identity task
...
task_identity="$(gluerun_graph_identity task "$treat_id")"
T="$(gluerun_graph_node_id "$task_identity")"
...
CON="$(gluerun_graph_node_id 'pv:CON')"

nodes_in="$tmp/nodes.in"; edges_in="$tmp/edges.in"
{
  gluerun_graph_emit_node task         "$task_identity" "$a_task"   'cT'
  gluerun_graph_emit_node attempt      'attempt:A'      'src/A'     'cA'
  gluerun_graph_emit_node plan-version 'pv:P'           'src/P'     'cP'
  gluerun_graph_emit_node critique     'crit:C'         "$a_crit"   'cC'
  gluerun_graph_emit_node assumption   'assume:AS'      "$a_assume" 'cAS'
  gluerun_graph_emit_node finding      'find:F'         "$a_find"   'cF'
  gluerun_graph_emit_node finding      'find:RO'        "$a_reject" 'cRO'
  gluerun_graph_emit_node decision     'disp:D'         "$a_dec"    'cD'
  gluerun_graph_emit_node plan-version 'pv:CON'         'src/CON'   'cCON'
} > "$nodes_in"
{
  ...
  gluerun_graph_emit_edge rejects_observation "$D"   "$RO" 'src/e7'  'c7'
  gluerun_graph_emit_edge revises             "$D"   "$T"  'src/e8'  'c8'
  gluerun_graph_emit_edge contradicts         "$CON" "$AS" 'src/e9'  'c9'
} > "$edges_in"
gluerun_graph_write_corpus "$GDIR" "$nodes_in" "$edges_in" || fail "setup: write_corpus failed"
...
# ===========================================================================
sel="$(gluerun_ctx_rehydrate_subgraph_select "$GDIR" "$T")" || fail "1: select nonzero exit"
...
# ===========================================================================
export GLUERUN_CTX_SUBGRAPH_REHYDRATE=1   # gates the WIRED selectors only.
packet="$(gluerun_ctx_rehydrate_subgraph_assemble "$GDIR" "$T" packet)" \
...
# ===========================================================================
manifest="$(gluerun_ctx_rehydrate_subgraph_assemble "$GDIR" "$T" manifest)" \
...
# ===========================================================================
# (4) Caps unchanged: shrinking GLUERUN_CONTEXT_SECTION_MAX_CHARS shrinks the big
...
small_packet="$(GLUERUN_CONTEXT_SECTION_MAX_CHARS=200 \
  gluerun_ctx_rehydrate_subgraph_assemble "$GDIR" "$T" packet)" \
...
small_packet2="$(GLUERUN_CONTEXT_SECTION_MAX_CHARS=200 \
  gluerun_ctx_rehydrate_subgraph_assemble "$GDIR" "$T" packet)" || fail "4: repeat shrunk assemble failed"
...
# ===========================================================================
packet2="$(gluerun_ctx_rehydrate_subgraph_assemble "$GDIR" "$T" packet)"   || fail "5: repeat packet failed"
manifest2="$(gluerun_ctx_rehydrate_subgraph_assemble "$GDIR" "$T" manifest)" || fail "5: repeat manifest failed"
...
# gluerun_ctx_route_subgraph_render (the shared inject/record selector).
r_packet="$(gluerun_ctx_route_subgraph_render "$treat_id" packet)" \
...
r_manifest="$(gluerun_ctx_route_subgraph_render "$treat_id" manifest)" \
...
# gluerun_ctx_route_subgraph_manifest ON -> the subgraph manifest at the same node.
route_m="$(gluerun_ctx_route_subgraph_manifest diff-volume implementer some-step "$meta" "$treat_id")" \
...
export GLUERUN_CTX_SUBGRAPH_REHYDRATE=1
gluerun_ctx_rehydrate_subgraph_assemble "$GDIR" "$T" packet   >/dev/null || fail "8: read-only assemble failed"
gluerun_ctx_rehydrate_subgraph_assemble "$GDIR" "$T" manifest >/dev/null || fail "8: read-only assemble(manifest) failed"
gluerun_ctx_route_subgraph_render "$treat_id" packet >/dev/null || fail "8: read-only render failed"
...
[[ -z "$(ls -A "$GLUERUN_STATE_DIR" 2>/dev/null)" ]] || fail "8: composed path wrote into the state dir"
