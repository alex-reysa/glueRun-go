...
# singular_session_resume_decide (task role, engine/lib.sh) and
# singular_planner_resume_decide (planner role, engine/ctx-planner-resume.sh) —
...
#   - OFF-parity: with SINGULAR_CTX_ROUTING unset/!=1 (default 0), for a given role
...
#   - ON, per-role strategy table: with SINGULAR_CTX_ROUTING=1 and a fixture where
...
#   - Taint / independence pin: with SINGULAR_CTX_ROUTING=1, at steps final-audit
...
# The spine must exist and define singular_ctx_route (RED before it is written).
...
[[ "$(type -t singular_session_resume_decide)" == "function" ]] \
  || fail "singular_session_resume_decide missing (task-role decider)"
[[ "$(type -t singular_planner_resume_decide)" == "function" ]] \
  || fail "singular_planner_resume_decide missing (planner-role decider)"
...
# --- Planner template under SINGULAR_ROOT so the template-sha gate finds it ----
...
planner_decide() { # <meta> <lineage_head>
  SINGULAR_PLANNER_SESSION=1 singular_planner_resume_decide "$1" node-1 codex-run.sh "$wt" "$2"
}
...
got="$(SINGULAR_CTX_ROUTING=0 route implementer implement "$m" TASK-1 RUN-1 codex-run.sh \
...
got="$(SINGULAR_CTX_ROUTING=0 SINGULAR_PLANNER_SESSION=1 route planner plan "$m" node-1 RUN-1 codex-run.sh \
...
lease_planner="$(singular_ctx_route_session_lease_path planner node-1)"
rm -f "$lease_task" "$lease_planner"

# 2a. All gates pass -> the decider's resume stands (task role).
m="$tmp/on-task.json"; mk_task "$m"
got="$(SINGULAR_CTX_ROUTING=1 route implementer implement "$m" TASK-1 RUN-1 codex-run.sh \
  "$PSHA" "$wt" "$HEAD2" "$TRANSCRIPT" TASK-1 "$HEAD1")"
...
got="$(SINGULAR_CTX_ROUTING=1 SINGULAR_SESSION_DIFF_MAX_LINES=0 route implementer implement "$m" \
...
got="$(SINGULAR_CTX_ROUTING=1 route implementer implement "$mf" TASK-1 RUN-1 codex-run.sh \
  "$PSHA" "$wt" "$HEAD2" "$TRANSCRIPT" TASK-1 "$HEAD1")"
...
got="$(SINGULAR_CTX_ROUTING=1 SINGULAR_PLANNER_SESSION=1 route planner plan "$mp" node-1 RUN-1 codex-run.sh \
  "$PSHA" "$wt" "$HEAD2" "$TRANSCRIPT" node-1 "$HEAD1")"
...
got="$(SINGULAR_CTX_ROUTING=1 SINGULAR_PLANNER_SESSION=1 SINGULAR_SESSION_DIFF_MAX_LINES=0 route planner plan \
...
assert_eq "$lease_planner" "$SINGULAR_STATE_DIR/sessions/planner/node-1.lease" \
...
for step in final-audit paired-audit; do
  got="$(SINGULAR_CTX_ROUTING=1 route implementer "$step" "$m" TASK-1 RUN-1 codex-run.sh \
    "$PSHA" "$wt" "$HEAD2" "$TRANSCRIPT" TASK-1 "$HEAD1")"
...
knob_out="$(env \
    ...
    bash -c '
      ...
      for step in final-audit paired-audit; do
        singular_ctx_route implementer "$step" "'"$m"'" TASK-1 RUN-1 codex-run.sh \
          "'"$PSHA"'" "'"$wt"'" "'"$HEAD2"'" "'"$TRANSCRIPT"'" TASK-1 "'"$HEAD1"'"
      done')"
...
got="$(SINGULAR_CTX_ROUTING=1 route implementer final-audit "$mfresh" TASK-1 RUN-1 codex-run.sh \
  "$PSHA" "$wt" "$HEAD2" "$TRANSCRIPT" TASK-1 "$HEAD1")"
...
rc=0; out="$(SINGULAR_CTX_ROUTING=1 route implementer implement "$m" TASK-1 RUN-1 codex-run.sh \
  "$PSHA" "$wt" "$HEAD2" "$TRANSCRIPT" TASK-1 "$HEAD1")" || rc=$?
assert_eq "$rc" "0" "router exit 0 (resume path)"
check_one_line "$out" "resume path"
rc=0; out="$(SINGULAR_CTX_ROUTING=1 route implementer final-audit "$m" TASK-1 RUN-1 codex-run.sh \
  "$PSHA" "$wt" "$HEAD2" "$TRANSCRIPT" TASK-1 "$HEAD1")" || rc=$?
