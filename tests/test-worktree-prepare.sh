...
# singular_worktree_prepare — the single path by which a worktree becomes
# runnable.
#
...
export SINGULAR_EVENTS_FILE="$root/.singular-state/events.ndjson"
...
[[ "$(SINGULAR_WORKTREE_COPY_PATHS_JSON='[]' singular_worktree_copy_paths_json)" \
   == '["node_modules"]' ]] || fail "c1: default is not node_modules"
got="$(SINGULAR_WORKTREE_COPY_PATHS_JSON='["apps/web/node_modules"]' singular_worktree_copy_paths_json)"
...
[[ "$(SINGULAR_AUDIT_VERIFY_COPY_PATHS_JSON='["vendor"]' singular_worktree_copy_paths_json)" \
   == '["node_modules","vendor"]' ]] || fail "c1: legacy env knob ignored"

# Escapes are still refused.
for bad in '["/abs"]' '["../up"]' '[".."]' '"notalist"'; do
  SINGULAR_WORKTREE_COPY_PATHS_JSON="$bad" singular_worktree_copy_paths_json >/dev/null 2>&1 \
...
SINGULAR_WORKTREE_COPY_PATHS_JSON='["apps/web/node_modules"]' \
  singular_worktree_copy_paths "$root" "$target" 2>"$tmp/copy.err"
grep -q "declared path is absent" "$tmp/copy.err" \
  || fail "c2: absent declared path was skipped in silence"
grep -q '"type":"worktree.copy_path_absent"' "$SINGULAR_EVENTS_FILE" \
...
export SINGULAR_PREWARM_CMD="printf warmed > prewarm.marker"
export SINGULAR_WORKTREE_COPY_PATHS_JSON='["apps/web/node_modules"]'
singular_worktree_prepare "$wt" "" "$root" "$tmp/prepare.log" \
  || fail "c3: prepare failed (stage=$SINGULAR_WORKTREE_PREPARE_STAGE): $(cat "$tmp/prepare.log")"
...
[[ -z "$SINGULAR_WORKTREE_PREPARE_STAGE" ]] || fail "c3: stage not cleared on success"
...
export SINGULAR_BOOTSTRAP_JSON='{"required":true,"commands":[{"command":"exit 7"}]}'
SINGULAR_WORKTREE_PREPARE_BOOTSTRAP_FATAL=yes \
  singular_worktree_prepare "$wt2" "" "" "$tmp/prepare2.log" \
  && fail "c4: a fatal bootstrap failure should fail prepare"
[[ "$SINGULAR_WORKTREE_PREPARE_STAGE" == "bootstrap" ]] \
  || fail "c4: failing stage not named (got: $SINGULAR_WORKTREE_PREPARE_STAGE)"
...
SINGULAR_WORKTREE_PREPARE_BOOTSTRAP_FATAL=no \
  singular_worktree_prepare "$wt3" "" "" "$tmp/prepare3.log" \
...
for site in l1-drive audit-verify accept-existing-packet; do
  grep -q "singular_worktree_prepare" "$ENGINE_HOME/engine/$site.sh" \
