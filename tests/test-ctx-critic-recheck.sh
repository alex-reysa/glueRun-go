...
# engine/ctx-critic-recheck.sh: a default-OFF SINGULAR_CRITIC_RECHECK_PCT knob
...
#   (b) OFF-by-default: SINGULAR_CRITIC_RECHECK_PCT unset OR "0" never samples any
...
[[ "$(type -t singular_ctx_critic_recheck_bucket)" == "function" ]] \
  || fail "singular_ctx_critic_recheck_bucket not defined (auto-source failed?)"
[[ "$(type -t singular_ctx_critic_recheck_should_sample)" == "function" ]] \
  || fail "singular_ctx_critic_recheck_should_sample not defined (auto-source failed?)"
...
for id in "${fixture[@]}"; do
  b1="$(singular_ctx_critic_recheck_bucket "$id")" || fail "bucket crashed for $id"
  [[ "$b1" =~ ^[0-9]+$ ]] || fail "bucket not numeric for $id: '$b1'"
  (( b1 >= 0 && b1 <= 99 )) || fail "bucket out of 0..99 for $id: $b1"
  b2="$(singular_ctx_critic_recheck_bucket "$id")" || fail "bucket repeat crashed for $id"
  [[ "$b1" == "$b2" ]] || fail "bucket not stable across calls for $id ($b1 vs $b2)"
  b3="$(bash -c 'source "'"$CTX_CR"'"; singular_ctx_critic_recheck_bucket "'"$id"'"')" \
...
unset SINGULAR_CRITIC_RECHECK_PCT
for id in "${fixture[@]}"; do
  if singular_ctx_critic_recheck_should_sample "$id"; then
    fail "OFF (unset): sampled id $id"
  fi
done

export SINGULAR_CRITIC_RECHECK_PCT=0
for id in "${fixture[@]}"; do
  if singular_ctx_critic_recheck_should_sample "$id"; then
...
[[ ! -s "$SINGULAR_EVENTS_FILE" ]] || fail "gate appended an event"

# ---------------------------------------------------------------------------
# (c) garbage PCT is fail-safe OFF.
# ---------------------------------------------------------------------------
for junk in "abc" "1.5" "-3" "10x" " " "1e2" "0x10" "nan"; do
  export SINGULAR_CRITIC_RECHECK_PCT="$junk"
  for id in "${fixture[@]}"; do
    if singular_ctx_critic_recheck_should_sample "$id"; then
...
# ---------------------------------------------------------------------------
export SINGULAR_CRITIC_RECHECK_PCT=100
for id in "${fixture[@]}"; do
...
for P in 25 50 73; do
  export SINGULAR_CRITIC_RECHECK_PCT="$P"
  sampled=0
  for id in "${fixture[@]}"; do
    bucket="$(singular_ctx_critic_recheck_bucket "$id")"
    want=1; (( bucket < P )) || want=0
    got=0; singular_ctx_critic_recheck_should_sample "$id" && got=1
    [[ "$want" == "$got" ]] \
      || fail "P=$P: id $id bucket=$bucket want=$want got=$got (subset != bucket<P)"
    (( got == 1 )) && sampled=$((sampled + 1))

    got2=0; singular_ctx_critic_recheck_should_sample "$id" && got2=1
    [[ "$got" == "$got2" ]] || fail "P=$P: repeated decision differs for $id"
    got3="$(SINGULAR_CRITIC_RECHECK_PCT="$P" bash -c \
      'source "'"$CTX_CR"'"; if singular_ctx_critic_recheck_should_sample "'"$id"'"; then echo 1; else echo 0; fi')" \
