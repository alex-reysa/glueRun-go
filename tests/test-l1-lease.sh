...
make_repo() {
  local root="$1"
  # NOTE: .gluerun-state/l1-leases is intentionally NOT pre-created — the lease
...
test_l1_lease_write_and_read() {
  ...
  assert_contains "$(gluerun_l1_lease_status S0.storage_substrate_base)" "proposed" "lease status round-trips"
  assert_contains "$(gluerun_l1_lease_field S0.storage_substrate_base allowedWriteScopes)" "internal/storage/" "lease defaults write scope to internal/<area>/"
}

test_l1_lease_schema_rejects_bad_status() {
  with_fixture
  local rc=0
  gluerun_l1_lease_write D1.contract artifact D1 contract bogus RUN-x abc1234 target 2>/dev/null || rc=$?
...
test_select_frontier_limit_respected() {
  with_fixture
  local out
  out="$(gluerun_select_l1_frontier 1)"
...
test_l1_stale_reported_not_reused() {
  ...
  stale="$(GLUERUN_L1_STALE_MINUTES=0 gluerun_l1_list_stale)"
  ...
  frontier="$(gluerun_select_l1_frontier 5)"
...
test_l1_lease_set_status_releases_slot() {
  with_fixture
  gluerun_l1_lease_write S0.storage_substrate_base storage S0 storage_substrate_base active RUN-x abc1234 target
  assert_contains "$(gluerun_l1_list_active)" "S0.storage_substrate_base" "active lease is listed"
  gluerun_l1_lease_set_status S0.storage_substrate_base released
  assert_not_contains "$(gluerun_l1_list_active)" "S0.storage_substrate_base" "released lease drops from active list"
  assert_contains "$(gluerun_select_l1_frontier 5)" "S0.storage_substrate_base" "releasing a lease frees the node for re-selection"
}
...
test_l1_lease_rejects_empty_node() {
  ...
  [[ ! -e "$GLUERUN_L1_LEASES_DIR/.json" ]] || fail "a rejected empty-node lease must not be written"
}

test_l1_lease_rejects_bad_basesha() {
  with_fixture
  local rc=0
  gluerun_l1_lease_write S0.storage_substrate_base storage S0 storage_substrate_base active RUN-x NOTHEX target 2>/dev/null || rc=$?
  [[ "$rc" -ne 0 ]] || fail "a non-hex baseSha must be rejected (schema pattern)"
  [[ ! -f "$(gluerun_l1_lease_path S0.storage_substrate_base)" ]] || fail "a rejected baseSha lease must not be written"
}
...
test_l1_lease_set_status_rejects_bogus() {
  ...
  lease="$(gluerun_l1_lease_path S0.storage_substrate_base)"
  before="$(shasum -a 256 "$lease" | awk '{print $1}')"
  gluerun_l1_lease_set_status S0.storage_substrate_base totally-bogus 2>/dev/null || rc=$?
  after="$(shasum -a 256 "$lease" | awk '{print $1}')"
  ...
  assert_contains "$(gluerun_l1_lease_status S0.storage_substrate_base)" "active" "status stays valid after a rejected update"
}
...
test_ensure_state_dirs_stays_dormant() {
  with_fixture
  [[ ! -d "$GLUERUN_L1_LEASES_DIR" ]] || fail "fixture must start without an l1-leases dir"
  gluerun_ensure_state_dirs
  [[ ! -d "$GLUERUN_L1_LEASES_DIR" ]] || fail "ensure_state_dirs must not create the l1-leases dir (strict dormancy)"
}
