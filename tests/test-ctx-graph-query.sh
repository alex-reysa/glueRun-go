...
for fn in gluerun_graph_query_neighbors gluerun_graph_query_lineage gluerun_graph_query_open_contradictions; do
...
U="$(gluerun_graph_node_id 'unrelated:U')"

{
  ...
  gluerun_graph_emit_node goal         'unrelated:U'  'src/U' 'cU'
} > "$NODES_A"
...
E_A_impl_T="$(gluerun_graph_edge_id "$A" implements "$T")"
E_P_df_A="$(gluerun_graph_edge_id "$P" derived_from "$A")"
E_C_crit_P="$(gluerun_graph_edge_id "$C" critiques "$P")"
E_F_df_C="$(gluerun_graph_edge_id "$F" derived_from "$C")"
E_D_acc_F="$(gluerun_graph_edge_id "$D" accepts_observation "$F")"
E_D_rev_T="$(gluerun_graph_edge_id "$D" revises "$T")"
E_U_con_T="$(gluerun_graph_edge_id "$U" contradicts "$T")"

{
  ...
  gluerun_graph_emit_edge derived_from        "$F" "$C" 'src/e4' 'c4'
  gluerun_graph_emit_edge accepts_observation "$D" "$F" 'src/e5' 'c5'
  gluerun_graph_emit_edge revises             "$D" "$T" 'src/e6' 'c6'
  gluerun_graph_emit_edge contradicts         "$U" "$T" 'src/e7' 'c7'
} > "$EDGES_A"

gluerun_graph_write_corpus "$GDIR_A" "$NODES_A" "$EDGES_A" || fail "write_corpus A failed"

# --- Neighbors of T: incident edges e1 (A->T), e6 (D->T), e7 (U->T); adjacent A,D,U.
neigh_T="$(gluerun_graph_query_neighbors "$GDIR_A" "$T")" || fail "neighbors(T) nonzero exit"
...
neigh_P="$(gluerun_graph_query_neighbors "$GDIR_A" "$P")" || fail "neighbors(P) nonzero exit"
...
neigh_unknown="$(gluerun_graph_query_neighbors "$GDIR_A" "$unknown")" \
...
neigh_T2="$(gluerun_graph_query_neighbors "$GDIR_A" "$T")" || fail "neighbors(T) repeat nonzero exit"
...
lin_T="$(gluerun_graph_query_lineage "$GDIR_A" "$T")" || fail "lineage(T) nonzero exit"
got_lin="$(printf '%s\n' "$lin_T" | kind_ids node)"
...
lin_C="$(gluerun_graph_query_lineage "$GDIR_A" "$C")" || fail "lineage(C) nonzero exit"
got_lin_C="$(printf '%s\n' "$lin_C" | kind_ids node)"
expect_eq "lineage(C) equals connected component" "$got_lin_C" "$want_lin"

# --- Lineage of an unknown start -> empty, zero exit.
lin_unknown="$(gluerun_graph_query_lineage "$GDIR_A" "$unknown")" \
...
X="$(gluerun_graph_node_id 'src:X')"
Y="$(gluerun_graph_node_id 'con:Y')"
Z="$(gluerun_graph_node_id 'con:Z')"
M="$(gluerun_graph_node_id 'src:M')"
N="$(gluerun_graph_node_id 'con:N')"
S="$(gluerun_graph_node_id 'sup:S')"

{
  gluerun_graph_emit_node plan-version 'src:X' 'src/X' 'cX'
  gluerun_graph_emit_node plan-version 'con:Y' 'src/Y' 'cY'
  gluerun_graph_emit_node plan-version 'con:Z' 'src/Z' 'cZ'
  gluerun_graph_emit_node plan-version 'src:M' 'src/M' 'cM'
  gluerun_graph_emit_node plan-version 'con:N' 'src/N' 'cN'
  gluerun_graph_emit_node plan-version 'sup:S' 'src/S' 'cS'
} > "$NODES_B"

E_X_con_Y="$(gluerun_graph_edge_id "$X" contradicts "$Y")"
E_X_inv_Z="$(gluerun_graph_edge_id "$X" invalidates "$Z")"
E_M_inv_N="$(gluerun_graph_edge_id "$M" invalidates "$N")"
E_S_sup_N="$(gluerun_graph_edge_id "$S" supersedes "$N")"

{
  gluerun_graph_emit_edge contradicts "$X" "$Y" 'src/b1' 'b1'
  gluerun_graph_emit_edge invalidates "$X" "$Z" 'src/b2' 'b2'
  gluerun_graph_emit_edge invalidates "$M" "$N" 'src/b3' 'b3'
  gluerun_graph_emit_edge supersedes  "$S" "$N" 'src/b4' 'b4'
} > "$EDGES_B"

gluerun_graph_write_corpus "$GDIR_B" "$NODES_B" "$EDGES_B" || fail "write_corpus B failed"

open_con="$(gluerun_graph_query_open_contradictions "$GDIR_B")" || fail "open_contradictions nonzero exit"
...
open_con2="$(gluerun_graph_query_open_contradictions "$GDIR_B")" || fail "open_contradictions repeat nonzero exit"
...
gluerun_graph_query_neighbors "$GDIR_A" "$T" >/dev/null || fail "read-only neighbors failed"
gluerun_graph_query_lineage "$GDIR_A" "$T" >/dev/null || fail "read-only lineage failed"
gluerun_graph_query_open_contradictions "$GDIR_A" >/dev/null || fail "read-only open_contradictions failed"
...
eres="$(gluerun_graph_query_neighbors "$GDIR_EMPTY" "$T")" || fail "neighbors on empty corpus must exit zero"
[[ -z "$eres" ]] || fail "neighbors on empty corpus must be empty"
eres="$(gluerun_graph_query_lineage "$GDIR_EMPTY" "$T")" || fail "lineage on empty corpus must exit zero"
[[ -z "$eres" ]] || fail "lineage on empty corpus must be empty"
eres="$(gluerun_graph_query_open_contradictions "$GDIR_EMPTY")" || fail "open_contradictions on empty corpus must exit zero"
