...
# recorded as a ctx.arm_assigned event ONLY when the default-OFF SINGULAR_CTX_AB
# knob is 1. Asserts:
#   (a) OFF (SINGULAR_CTX_AB unset AND =0) -> zero ctx.arm_assigned events, no
...
#   (d) ON (SINGULAR_CTX_AB=1) -> each assignment appends exactly one
#       ctx.arm_assigned event whose data carries the task id and its arm.
# The events log is pinned to an isolated temp file (SINGULAR_EVENTS_FILE) so the
...
# lib.sh provides singular_append_event; source it against an isolated root so no
...
[[ "$(type -t singular_ctx_ab_arm_for)" == "function" ]] \
  || fail "singular_ctx_ab_arm_for is not defined by $CTX_AB"
[[ "$(type -t singular_ctx_ab_assign)" == "function" ]] \
  || fail "singular_ctx_ab_assign is not defined by $CTX_AB"

# Point the events log at an isolated temp file. lib.sh sets SINGULAR_EVENTS_FILE
...
count_arm_events() {
  ...
  c="$(grep -c '"type":"ctx.arm_assigned"' "$SINGULAR_EVENTS_FILE" 2>/dev/null)" || true
  echo "${c:-0}"
}
...
# ---------------------------------------------------------------------------
# Unset knob.
unset SINGULAR_CTX_AB
: > "$SINGULAR_EVENTS_FILE"
before="$(shasum "$SINGULAR_EVENTS_FILE" | awk '{print $1}')"
arm_off="$(singular_ctx_ab_assign "TASK-0004")" || fail "assign crashed with knob unset"
[[ "$arm_off" == "A" || "$arm_off" == "B" ]] || fail "OFF: arm not in {A,B}: [$arm_off]"
[[ "$(count_arm_events)" -eq 0 ]] || fail "OFF (unset): ctx.arm_assigned event emitted"
after="$(shasum "$SINGULAR_EVENTS_FILE" | awk '{print $1}')"
[[ "$before" == "$after" ]] || fail "OFF (unset): events log mutated"

# Knob explicitly 0.
SINGULAR_CTX_AB=0
before="$(shasum "$SINGULAR_EVENTS_FILE" | awk '{print $1}')"
singular_ctx_ab_assign "TASK-0004" >/dev/null || fail "assign crashed with knob=0"
...
# ---------------------------------------------------------------------------
a1="$(singular_ctx_ab_arm_for "TASK-0004")"
a2="$(singular_ctx_ab_arm_for "TASK-0004")"
...
a3="$(bash -c 'source "'"$CTX_AB"'"; singular_ctx_ab_arm_for "TASK-0004"')" \
...
for id in "${fixture[@]}"; do
  ...
  for _ in 1 2 3; do
    again="$(singular_ctx_ab_arm_for "$id")"
...
# ---------------------------------------------------------------------------
: > "$SINGULAR_EVENTS_FILE"
export SINGULAR_CTX_AB=1
declare -a expect_ids=()
declare -a expect_arms=()
n=0
for id in "${fixture[@]}"; do
  arm="$(singular_ctx_ab_assign "$id")" || fail "ON: assign crashed for $id"
  # Arm printed by assign must equal the pure mapping.
  pure="$(singular_ctx_ab_arm_for "$id")"
...
python3 - "$SINGULAR_EVENTS_FILE" "${expect_ids[@]}" "--arms--" "${expect_arms[@]}" <<'PY' \
