...
cat >"$wrapper" <<'EOF'
#!/bin/sh
printf 'called\n' >>"$GLUERUN_TEST_BASH_MARKER"
printf '%s\n' "$PATH" >"$GLUERUN_TEST_PATH_MARKER"
exec "$GLUERUN_TEST_REAL_BASH" "$@"
EOF
chmod +x "$wrapper"

original_path="$PATH"
out="$(
  GLUERUN_BASH_BIN="$wrapper" \
  GLUERUN_TEST_BASH_MARKER="$marker" \
  GLUERUN_TEST_PATH_MARKER="$path_marker" \
  GLUERUN_TEST_REAL_BASH="$BASH" \
  GLUERUN_ENGINE_HOME="$ENGINE_HOME" \
  "$BASH" "$ENGINE_HOME/cli/gluerun" status
)"
assert_contains "$out" "gluerun orchestration status" "CLI reaches the engine under the selected Bash"
...
rc=0
out="$(GLUERUN_BASH_BIN=bash "$BASH" "$ENGINE_HOME/cli/gluerun" version 2>&1)" || rc=$?
[[ "$rc" -ne 0 ]] || fail "relative GLUERUN_BASH_BIN must be rejected"
