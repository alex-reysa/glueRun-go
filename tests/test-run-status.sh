...
printf '%s\n' '{"schema":"singular.orchestration.dag.v0","nodes":[{"id":"health-fixture","stage":"S0","area":"test","layer":"test","kind":"contract","dependsOn":[],"requiredCompletion":"done"}]}' \
...
env "${env_base[@]}" SINGULAR_NOW="2026-07-24T10:00:00Z" \
...
env "${env_base[@]}" SINGULAR_NOW="2026-07-24T10:01:00Z" \
  "$ROOT/engine/run-status.sh" write \
...
status="$repo/.singular-state/runs/RUN-1/run-status.json"
python3 - "$status" <<'PY'
import json, sys
...
pidfile="$repo/.singular-state/autonomate.pid"
printf '%s\n' "$$" >"$pidfile"
...
printf '%s\n' "$$" >"$pidfile"
health_lone_override="$(env "${env_base[@]}" SINGULAR_TEST_PID_PROBE_STATE=unknown \
  "$ROOT/engine/ops.sh" health --json)"
health_invalid_override="$(env "${env_base[@]}" SINGULAR_TEST_PID_PROBE=1 \
  SINGULAR_TEST_PID_PROBE_STATE=not-a-state "$ROOT/engine/ops.sh" health --json)"
...
printf '%s\n' "$$" >"$pidfile"
health_unknown="$(env "${env_base[@]}" SINGULAR_TEST_PID_PROBE=1 \
  SINGULAR_TEST_PID_PROBE_STATE=unknown "$ROOT/engine/ops.sh" health --json)"
...
health_unknown_human="$(env "${env_base[@]}" SINGULAR_TEST_PID_PROBE=1 \
  SINGULAR_TEST_PID_PROBE_STATE=unknown "$ROOT/engine/ops.sh" health)"
