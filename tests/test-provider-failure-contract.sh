...
tmp="$(mktemp -d "${TMPDIR:-/tmp}/singular-provider-result.XXXXXX")"
trap 'rm -rf "$tmp"' EXIT
...
write_result() {
  ...
  local dir="$SINGULAR_RUNS_DIR/RUN-$provider"
  mkdir -p "$dir"
  printf '%s\n' "$envelope_json" >"$dir/envelope.json"
  singular_runner_result_write "$provider" "RUN-$provider" planner planner-core \
...
doc = json.loads(raw)
assert doc["schema"] == "singular.runner-contract.v1"
...
if [[ "${1:-}" == "--describe-contract" ]]; then
  printf '%s\n' '{"schema":"singular.runner-contract.v1","version":1,"provider":"custom","arguments":["--worktree","--prompt-file","--level","--run-id","--role","--capability-profile","--result-file","--describe-contract"],"structuredResult":"singular.orchestration.runner-result.v0","structuredProviderError":"singular.orchestration.provider-error.v0"}'
  exit 0
fi
...
CONTRACT_CAPTURE="$contract_capture" \
  "$contract_runner" "${SINGULAR_RUNNER_CONTRACT_ARGS[@]}" --worktree "$repo"
...
for provider in codex claude gemini opencode cursor openrouter grok; do
  ...
  evidence="$(singular_runner_quota_evidence_json "$result" "$expected")" \
  ...
  singular_runner_quota_evidence_json "$result" "$other" >/dev/null 2>&1 \
    && fail "$provider $expected evidence cross-validated as $other"
  singular_runner_quota_evidence_json "$result" any >/dev/null 2>&1 \
...
PY
singular_runner_quota_evidence_json "$result" >/dev/null 2>&1 \
...
singular_runner_quota_evidence_json "$result" >/dev/null \
...
[[ "$(singular_planner_failure_class "$legacy" 1 "$tmp/missing-output" "$tmp/no-result.json")" == "codex-exit" ]] \
  || fail "legacy runner prose changed failure class"
if singular_planner_backoff_set quota RUN-legacy planner "$legacy" 2>/dev/null; then
  fail "legacy runner log armed quota backoff"
fi
[[ ! -f "$SINGULAR_PLANNER_BACKOFF_FILE" ]] || fail "rejected legacy evidence wrote backoff"
grep -q '"type":"backoff.rejected_invalid_evidence"' "$SINGULAR_EVENTS_FILE" \
...
singular_planner_backoff_set quota RUN-codex planner "$valid" \
  || fail "valid structured evidence did not arm backoff"
[[ "$(json_field "$SINGULAR_PLANNER_BACKOFF_FILE" evidenceRef)" == "$valid" ]] \
  || fail "backoff not bound to runner result"
[[ "$(json_field "$SINGULAR_PLANNER_BACKOFF_FILE" httpStatus)" == "429" ]] \
  || fail "backoff missing normalized status"
backoff_snapshot="$tmp/codex-planner-backoff.json"
cp "$SINGULAR_PLANNER_BACKOFF_FILE" "$backoff_snapshot"
SINGULAR_RUNNER="$ENGINE_HOME/engine/codex-run.sh" \
  singular_planner_backoff_active_json >/dev/null \
  || fail "matching built-in provider did not retain active backoff"
if SINGULAR_RUNNER="$ENGINE_HOME/engine/claude-run.sh" \
...
cmp -s "$SINGULAR_PLANNER_BACKOFF_FILE" "$backoff_snapshot" \
...
if singular_planner_backoff_set quota RUN-overload planner "$overloaded" 2>/dev/null; then
  fail "overload evidence armed a quota backoff"
fi
[[ ! -f "$SINGULAR_PLANNER_BACKOFF_FILE" ]] || fail "refused overload-as-quota still wrote a backoff"
singular_planner_backoff_set provider-overloaded RUN-overload planner "$overloaded" \
  || fail "overload evidence did not arm a provider-overloaded backoff"
[[ "$(json_field "$SINGULAR_PLANNER_BACKOFF_FILE" failureClass)" == "provider-overloaded" ]] \
  || fail "overload backoff recorded the wrong class"
[[ "$(json_field "$SINGULAR_PLANNER_BACKOFF_FILE" httpStatus)" == "529" ]] \
...
cp "$SINGULAR_PLANNER_BACKOFF_FILE" "$collision_snapshot"
SINGULAR_RUNNER="$collision_runner" \
  singular_planner_backoff_active_json >/dev/null \
  || fail "basename-colliding custom runner bypassed existing provider backoff"
cmp -s "$SINGULAR_PLANNER_BACKOFF_FILE" "$collision_snapshot" \
...
backoff_window="$(python3 - "$SINGULAR_PLANNER_BACKOFF_FILE" <<'PY'
import json, sys
...
PY
singular_runner_quota_evidence_json "$forged" quota >/dev/null 2>&1 \
  && fail "a result claiming quota over an overload envelope passed the quota query"
singular_runner_quota_evidence_json "$forged" any >/dev/null 2>&1 \
  && fail "class/kind disagreement passed the any-window query"
if singular_planner_backoff_set quota RUN-forged planner "$forged" 2>/dev/null; then
  fail "forged quota class armed the long quota backoff from a 529"
fi
[[ ! -f "$SINGULAR_PLANNER_BACKOFF_FILE" ]] || fail "forged evidence wrote a backoff"
...
PY
singular_runner_quota_evidence_json "$valid" >/dev/null 2>&1 \
...
# singular_runner_quota_evidence_json. These cases pin what may and may not move
...
write_tagged_result() {
  ...
  local dir="$SINGULAR_RUNS_DIR/RUN-$tag"
  mkdir -p "$dir"
  printf '%s\n' "$envelope_json" >"$dir/envelope.json"
  singular_runner_result_write "$provider" "RUN-$tag" planner planner-core \
    "$dir/runner-result.json" "$exit_code" "$dir/envelope.json" "" "$dir/out.json"
  printf '%s\n' "$dir/runner-result.json"
}
pressure_field() {
...
rm -f "$SINGULAR_PLANNER_BACKOFF_FILE" "$SINGULAR_PROVIDER_PRESSURE_FILE"
...
singular_planner_backoff_set quota RUN-pressure-off planner "$quota_a" >/dev/null \
...
singular_provider_pressure_observe "$quota_a" >/dev/null 2>&1 \
  && fail "observe ran while adaptation was disabled"
singular_provider_pressure_status_json >/dev/null 2>&1 \
...
replay="$(singular_provider_pressure_observe "$quota_a")" \
...
singular_provider_pressure_observe "$prose_log" >/dev/null 2>&1 \
...
PY
singular_provider_pressure_observe "$tampered" >/dev/null 2>&1 \
...
entitlement="$(write_tagged_result claude pressure-entitlement \
  '{"type":"result","subtype":"error","is_error":true,"api_error_status":403,"result":"Organization has disabled subscription access for Claude Code."}')"
singular_runner_quota_evidence_json "$entitlement" any >/dev/null \
  || fail "entitlement fixture is not valid evidence"
singular_provider_pressure_observe "$entitlement" >/dev/null 2>&1 \
...
second="$(singular_provider_pressure_observe "$quota_b")" \
...
grep -q '"type":"provider_pressure.reduced"' "$SINGULAR_EVENTS_FILE" \
...
singular_provider_pressure_observe "$quota_a" >/dev/null \
...
switch_status="$(SINGULAR_RUNNER="$ENGINE_HOME/engine/claude-run.sh" \
...
collision_status="$(SINGULAR_RUNNER="$collision_runner" \
  singular_provider_pressure_status_json)" \
...
SINGULAR_RUNNER="$collision_runner" singular_provider_pressure_success >/dev/null 2>&1 \
...
[[ "$(pressure_field "$(singular_provider_pressure_success)" action)" == "quiet" ]] \
...
recovered="$(singular_provider_pressure_success)" || fail "quiet-interval success failed"
...
singular_provider_pressure_success >/dev/null || fail "quiet success failed"
cleared="$(singular_provider_pressure_success)" || fail "quiet success failed"
...
singular_provider_pressure_success >/dev/null 2>&1
[[ "$(pressure_cap codex)" == "null" ]] \
...
SINGULAR_PROVIDER_PRESSURE_FILE="$floor_state" SINGULAR_MAX_CONCURRENT=1 \
  singular_provider_pressure_observe "$floor_a" >/dev/null || fail "floor observe a failed"
floor_result="$(SINGULAR_PROVIDER_PRESSURE_FILE="$floor_state" SINGULAR_MAX_CONCURRENT=1 \
  singular_provider_pressure_observe "$floor_b")" || fail "floor observe b failed"
...
for suffix in c d; do
  ...
  SINGULAR_PROVIDER_PRESSURE_FILE="$floor_state" SINGULAR_MAX_CONCURRENT=1 \
    singular_provider_pressure_observe "$extra" >/dev/null || fail "floor observe $suffix failed"
done
[[ "$(SINGULAR_PROVIDER_PRESSURE_FILE="$floor_state" pressure_cap grok)" == "1" ]] \
...
floor_reduced="$(grep -c '"type":"provider_pressure.reduced"' "$SINGULAR_EVENTS_FILE" || true)"
...
SINGULAR_PROVIDER_PRESSURE_FILE="$floor_state" SINGULAR_MAX_CONCURRENT=1 \
  singular_provider_pressure_observe "$floor_e" >/dev/null || fail "floor observe e failed"
# floor_f completes a fresh cluster against a cap that is already at the floor.
held="$(SINGULAR_PROVIDER_PRESSURE_FILE="$floor_state" SINGULAR_MAX_CONCURRENT=1 \
  singular_provider_pressure_observe "$floor_f")" || fail "floor observe f failed"
[[ "$(pressure_field "$held" action)" == "held" ]] \
  || fail "a cluster that cannot lower the cap reported '$(pressure_field "$held" action)'"
[[ "$(grep -c '"type":"provider_pressure.reduced"' "$SINGULAR_EVENTS_FILE" || true)" == "$floor_reduced" ]] \
...
printf '{"schema":"singular.orchestration.provider-pressure.v0","providers":' >"$corrupt_state"
corrupt_status="$(SINGULAR_PROVIDER_PRESSURE_FILE="$corrupt_state" \
  singular_provider_pressure_status_json)" || fail "corrupt pressure state broke status"
[[ "$(pressure_field "$corrupt_status" cap)" == "null" ]] \
  || fail "corrupt pressure state produced a cap"
printf '%s' '{"schema":"singular.orchestration.provider-pressure.v0","providers":{"codex":{"cap":0,"events":"nope","quietSuccesses":-4}}}' >"$corrupt_state"
hostile_status="$(SINGULAR_PROVIDER_PRESSURE_FILE="$corrupt_state" \
  singular_provider_pressure_status_json)" || fail "out-of-range pressure state broke status"
...
rm -f "$SINGULAR_PROVIDER_PRESSURE_FILE"
relative_src="$SINGULAR_RUNS_DIR/RUN-pressure-rel"
...
PY
singular_runner_quota_evidence_json "$rel_result" any >/dev/null \
  || fail "relative-ref evidence fixture is not valid evidence"
cp -R "$relative_src" "$SINGULAR_RUNS_DIR/RUN-pressure-rel-copy"
singular_provider_pressure_observe "$rel_result" >/dev/null \
  || fail "relative-ref evidence was not observed"
singular_provider_pressure_observe \
  "$SINGULAR_RUNS_DIR/RUN-pressure-rel-copy/runner-result.json" >/dev/null \
...
for ev in "$floor_high_a" "$floor_high_b"; do
  SINGULAR_MAX_CONCURRENT=2 SINGULAR_PROVIDER_PRESSURE_MIN_SLOTS=6 \
    singular_provider_pressure_observe "$ev" >/dev/null \
    || fail "high-floor observe failed"
done
high_floor_cap="$(SINGULAR_MAX_CONCURRENT=2 pressure_cap gemini)"
[[ "$high_floor_cap" == "2" ]] \
  || fail "a floor above the configured ceiling persisted an unusable cap ($high_floor_cap)"
SINGULAR_MAX_CONCURRENT=2 SINGULAR_PROVIDER_PRESSURE_MIN_SLOTS=6 \
  singular_provider_pressure_observe "$floor_high_a" >/dev/null \
  || fail "high-floor replay failed"
[[ "$(SINGULAR_MAX_CONCURRENT=2 pressure_cap gemini)" == "2" ]] \
...
SINGULAR_PROVIDER_PRESSURE_FILE="$readonly_state" SINGULAR_MAX_CONCURRENT=4 \
  singular_provider_pressure_observe "$ro_a" >/dev/null || fail "readonly observe a failed"
reduced_before="$(grep -c '"type":"provider_pressure.reduced"' "$SINGULAR_EVENTS_FILE" || true)"
chmod 500 "$readonly_dir"
ro_report="$(SINGULAR_PROVIDER_PRESSURE_FILE="$readonly_state" SINGULAR_MAX_CONCURRENT=4 \
  singular_provider_pressure_observe "$ro_b")" || true
...
[[ "$(SINGULAR_PROVIDER_PRESSURE_FILE="$readonly_state" pressure_cap grok)" == "null" ]] \
  || fail "a failed write left a phantom cap on disk"
reduced_after="$(grep -c '"type":"provider_pressure.reduced"' "$SINGULAR_EVENTS_FILE" || true)"
...
grep -q '"type":"provider_pressure.write_failed"' "$SINGULAR_EVENTS_FILE" \
...
rm -f "$SINGULAR_PROVIDER_PRESSURE_FILE" "$SINGULAR_PLANNER_BACKOFF_FILE"
...
SINGULAR_MAX_CONCURRENT=4 singular_planner_backoff_set quota RUN-choke-a planner "$chokepoint_a" \
  || fail "chokepoint arming a failed"
SINGULAR_MAX_CONCURRENT=4 singular_planner_backoff_set quota RUN-choke-b planner "$chokepoint_b" \
...
rm -f "$SINGULAR_PROVIDER_PRESSURE_FILE"
if singular_planner_backoff_set quota RUN-choke-legacy planner "$legacy" 2>/dev/null; then
