#!/usr/bin/env bash
# Covers the plan-revision-loop RE-CRITIQUE resume authority brick
# engine/ctx-plan-recritic-resume.sh: the fail-closed, skeptic-role-gated
# decider that decides whether a re-critique may RESUME the persisted per-node
# plan-critic session (its prior concerns the checklist) or must run FRESH, plus
# the recorder that makes the carry-over observable. Implements the stage-file
# deliverable "revised candidates re-enter the critic (same critic session where
...
CTX="$ENGINE_HOME/engine/ctx-plan-recritic-resume.sh"
...
# ---------------------------------------------------------------------------
# Gate: role-mismatch (skeptic-role gate). A re-critique may ONLY re-enter a
# plan-critic session; any other role is a different lineage and never resumes.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Gate: head-rewritten (headShaAtCreate not an ancestor of the lineage head).
# ---------------------------------------------------------------------------
...
# plan-critic) carrying node, runId, revisesRunId, strategy, reason, sessionId.
# ---------------------------------------------------------------------------
...
PY
pass "record_strategy(resume): one plan-critic strategy_selected event with revisesRunId + sessionId"
...
echo "ctx-plan-recritic-resume tests passed"
