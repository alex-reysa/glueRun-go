#!/usr/bin/env bash
# Covers the sanctioned `cli/singular metrics` subcommand: a thin, read-only hook
# that delegates into the already-integrated engine/ctx-metrics.sh extractor
# (singular_ctx_metrics_json). Asserts the subcommand emits byte-identical output
# to the extractor over the same fixture runs dir + events file, mutates none of
# its inputs, writes no real .singular-state, fails safe on empty/missing inputs,
...
# bits under test. Pinning SINGULAR_ENGINE_HOME to the live repo would make the
# no-side-effect assertion below vacuous-or-false depending on where the suite
# runs (a pristine task worktree has no .singular-state; the ops tree, where
...
cp "$SINGULAR" "$EHOME/cli/singular"
...
( set -uo pipefail
  ...
  source "$EXTRACTOR"
  singular_ctx_metrics_json "$runs" "$events"
...
# --- Delegation: `singular metrics --json` == extractor bytes -----------------
"$SINGULAR" metrics --json --runs-dir "$runs" --events-file "$events" \
  > "$tmp/cli.json" 2>"$tmp/cli.err" \
  || fail "singular metrics --json exited non-zero: $(cat "$tmp/cli.err")"

cmp -s "$tmp/expected.json" "$tmp/cli.json" \
  || fail "singular metrics output not byte-identical to extractor: $(diff "$tmp/expected.json" "$tmp/cli.json" | head)"
...
m = json.loads(raw)
assert m["schema"] == "singular.orchestration.ctx-metrics.v0", m.get("schema")
...
# --- Read-only: inputs byte-unchanged; no real .singular-state written --------
...
[[ ! -e "$EHOME/.singular-state" ]] || fail "metrics wrote a real .singular-state artifact"

# --- Fail-safe: empty/missing runs dir + events file -> zeroed, exit 0 -------
"$SINGULAR" metrics --json --runs-dir "$tmp/no-such-runs" --events-file "$tmp/no-such.ndjson" \
...
"$SINGULAR" help > "$tmp/help.txt" 2>&1 || fail "singular help exited non-zero"
grep -q '^  metrics' "$tmp/help.txt" || fail "metrics not listed in usage()"

# Unknown-command path is unchanged (still errors, non-zero).
if "$SINGULAR" definitely-not-a-command >/dev/null 2>&1; then
