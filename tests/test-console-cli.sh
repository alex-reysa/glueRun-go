...
# with file cleanup, stale-file recovery, and the `gluerun status` console line.
# Hermetic: runs against a mktemp fixture repo with GLUERUN_ENGINE_HOME set to
...
cleanup() {
  ...
  local p
  for p in "$root/.gluerun-state/console.pid" "$tmp"/pids/*; do
    [[ -f "$p" ]] || continue
...
record_pid() { # remember a started server pid for the trap
  [[ -f "$root/.gluerun-state/console.pid" ]] \
    && cp "$root/.gluerun-state/console.pid" "$tmp/pids/$(date +%s%N 2>/dev/null || date +%s).$RANDOM"
}
...
[[ -f "$root/.gluerun-state/console.url" ]] || fail "console.url not written"
[[ "$(head -1 "$root/.gluerun-state/console.url")" == "$url" ]] \
  || fail "console.url content does not match printed url"
[[ -f "$root/.gluerun-state/console.pid" ]] || fail "console.pid not written"
pid="$(tr -d '[:space:]' < "$root/.gluerun-state/console.pid")"
...
url2="$("$GLUERUN" console --ensure)" || fail "second --ensure failed"
[[ "$url2" == "$url" ]] || fail "second --ensure changed the url ($url -> $url2)"
pid2="$(tr -d '[:space:]' < "$root/.gluerun-state/console.pid")"
[[ "$pid2" == "$pid" ]] || fail "second --ensure restarted the server (pid $pid -> $pid2)"

# --- 5. --status while running: exit 0 + url; --json shape ----------------------
out="$("$GLUERUN" console --status)" || fail "console --status should exit 0 while running"
assert_contains "$out" "$url" "status-running"
outj="$("$GLUERUN" console --status --json)" || fail "console --status --json should exit 0"
...
# --- 6. gluerun status prints the console line while running --------------------
stat="$("$GLUERUN" status 2>/dev/null)" || fail "gluerun status failed"
assert_contains "$stat" "console: $url" "gluerun-status-console-line"

# --- 7. --stop: server gone, files removed, idempotent --------------------------
"$GLUERUN" console --stop 2>/dev/null || fail "console --stop exited non-zero"
[[ ! -f "$root/.gluerun-state/console.url" ]] || fail "console.url not removed by --stop"
[[ ! -f "$root/.gluerun-state/console.pid" ]] || fail "console.pid not removed by --stop"
sleep 0.5
kill -0 "$pid" 2>/dev/null && fail "server pid $pid still alive after --stop"
"$GLUERUN" console --stop 2>/dev/null || fail "second --stop (idempotent) exited non-zero"
if "$GLUERUN" console --status >/dev/null 2>&1; then
  fail "--status should exit 1 after --stop"
fi

# --- 8. stale-file recovery -------------------------------------------------------
# `gluerun status` flags a dead console; --ensure cleans the leftovers and starts fresh.
echo "http://127.0.0.1:1" > "$root/.gluerun-state/console.url"   # nothing listens on :1
echo "999999" > "$root/.gluerun-state/console.pid"               # never a live pid
stat2="$("$GLUERUN" status 2>/dev/null)" || fail "gluerun status failed on stale files"
assert_contains "$stat2" "console: stale (not running)" "gluerun-status-stale-line"
url3="$("$GLUERUN" console --ensure)" || fail "--ensure did not recover from stale files"
record_pid
[[ "$url3" != "http://127.0.0.1:1" ]] || fail "--ensure reused the stale url"
pid3="$(tr -d '[:space:]' < "$root/.gluerun-state/console.pid")"
kill -0 "$pid3" 2>/dev/null || fail "recovered console pid ($pid3) not alive"
"$GLUERUN" console --stop 2>/dev/null || fail "final --stop exited non-zero"
