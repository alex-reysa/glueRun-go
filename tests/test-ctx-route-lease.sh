...
# (gluerun_planner_resume_lease_path / _live, hard-coded to
...
#     ${GLUERUN_STATE_DIR:-$GLUERUN_ROOT/.gluerun-state}. Empty role OR empty key
...
[[ "$(type -t gluerun_ctx_route_session_lease_path)" == "function" ]] \
  || fail "gluerun_ctx_route_session_lease_path not defined by $CTX_L"
[[ "$(type -t gluerun_ctx_route_session_lease_live)" == "function" ]] \
  || fail "gluerun_ctx_route_session_lease_live not defined by $CTX_L"

# --- _path: canonical role-generic lease path --------------------------------
out="$(gluerun_ctx_route_session_lease_path "implementer" "some-node")"
assert_eq "$out" "$GLUERUN_STATE_DIR/sessions/implementer/some-node.lease" \
  "path is <state-dir>/sessions/<role>/<key>.lease"
out="$(gluerun_ctx_route_session_lease_path "planner" "node-1")"
assert_eq "$out" "$GLUERUN_STATE_DIR/sessions/planner/node-1.lease" \
  "path for planner role"

# --- _path: default state-dir when GLUERUN_STATE_DIR unset --------------------
out="$(env -u GLUERUN_STATE_DIR bash -c '
  source "'"$LIB"'"; source "'"$CTX_L"'"
  gluerun_ctx_route_session_lease_path "auditor" "n2"')"
assert_eq "$out" "$GLUERUN_ROOT/.gluerun-state/sessions/auditor/n2.lease" \
  "path defaults state-dir to \$GLUERUN_ROOT/.gluerun-state"

# --- _path: empty role OR empty key -> empty string ---------------------------
assert_eq "$(gluerun_ctx_route_session_lease_path "" "k")" "" "empty role -> empty"
assert_eq "$(gluerun_ctx_route_session_lease_path "r" "")" "" "empty key -> empty"
assert_eq "$(gluerun_ctx_route_session_lease_path "" "")" "" "both empty -> empty"
pass "path: canonical role-generic path; empty role/key -> empty"

# --- _live: no file -> not-live (free) ---------------------------------------
rc=0; gluerun_ctx_route_session_lease_live "$tmp/nope.lease" || rc=$?
assert_eq "$rc" "1" "missing lease file -> not-live (free)"
rc=0; gluerun_ctx_route_session_lease_live "" || rc=$?
assert_eq "$rc" "1" "empty lease path -> not-live (free)"

# --- _live: live PID -> live (held) ------------------------------------------
lp="$GLUERUN_STATE_DIR/sessions/implementer/live.lease"
mkdir -p "$(dirname "$lp")"
printf '{"pid": %s}\n' "$$" >"$lp"   # our own PID is definitely alive
rc=0; gluerun_ctx_route_session_lease_live "$lp" || rc=$?
...
( set -e
  source "$LIB"; source "$CTX_L"
  gluerun_ctx_route_session_lease_live "$tmp/nope.lease" || true
  gluerun_ctx_route_session_lease_live "$lp" || true
...
# helper and gluerun_planner_resume_lease_path / _live.
source "$CTX_PLANNER" || fail "sourcing $CTX_PLANNER failed"
node="planner-node-x"
generic_path="$(gluerun_ctx_route_session_lease_path "planner" "$node")"
planner_path="$(gluerun_planner_resume_lease_path "$node")"
assert_eq "$generic_path" "$planner_path" "generic path == planner path for planner role/key"

mkdir -p "$(dirname "$planner_path")"
# live holder
printf '{"pid": %s}\n' "$$" >"$planner_path"
g=0; gluerun_ctx_route_session_lease_live "$generic_path" || g=$?
p=0; gluerun_planner_resume_lease_live "$planner_path" || p=$?
