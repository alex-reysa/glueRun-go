...
# engine/ctx-planner-session.sh: gluerun_ctx_planner_session_finalize must store,
...
# GLUERUN_PLANNER_TEMPLATE) — NOT the sha of the RENDERED per-frontier prompt its
# call site passes. This aligns both sides on the TEMPLATE sha so that a finalized
# planner meta round-trips through gluerun_planner_resume_decide (gate 8, which
...
#   (b) GLUERUN_PLANNER_TEMPLATE override is honored (same canonical convention as
#       the decider's gluerun_planner_resume_template_path).
...
# The planner template lives under GLUERUN_ROOT so the finalize wrapper (and the
...
RENDERED_A_SHA="$(gluerun_sha256_file "$render_a")"
...
RENDERED_B_SHA="$(gluerun_sha256_file "$render_b")"
...
seed_provider() { # <session_id>
  gluerun_session_meta_write_provider "$canon" codex "$1" m e "$wt" 0
}
...
seed_provider "SID-A"
gluerun_ctx_planner_session_finalize "$NODE" 0 "TASK-0010" "RUN-A" \
...
# ---------------------------------------------------------------------------
# (b) GLUERUN_PLANNER_TEMPLATE override honored (same convention as the decider).
# ---------------------------------------------------------------------------
alt_tpl="$tmp/alt-template.md"
printf 'ALTERNATE planner template body\n' > "$alt_tpl"
ALT_SHA="$(gluerun_sha256_file "$alt_tpl")"
[[ "$ALT_SHA" != "$TPL_SHA" ]] || fail "fixture bug: alt template sha equals default"
rm -f "$canon"; seed_provider "SID-ALT"
GLUERUN_PLANNER_TEMPLATE="$alt_tpl" gluerun_ctx_planner_session_finalize "$NODE" 0 \
  "TASK-0010" "RUN-ALT" "codex-run.sh" "$RENDERED_A_SHA" "$HEAD2" 1 \
  || fail "(b): finalize crashed"
STORED="$(python3 -c 'import json,sys;print(json.load(open(sys.argv[1])).get("promptSha256",""))' "$canon")"
assert_eq "$STORED" "$ALT_SHA" "(b) GLUERUN_PLANNER_TEMPLATE override drives the stored sha"
pass "(b) GLUERUN_PLANNER_TEMPLATE override honored (canonical template-path convention)"

# ---------------------------------------------------------------------------
# (c) finalize -> decide round-trips to resume with every other gate satisfied.
# ---------------------------------------------------------------------------
lease_path="$(gluerun_planner_resume_lease_path "$NODE")"; rm -f "$lease_path"
rm -f "$canon"; seed_provider "SID-RT"
gluerun_ctx_planner_session_finalize "$NODE" 0 "TASK-0010" "RUN-RT" \
  "codex-run.sh" "$RENDERED_A_SHA" "$HEAD2" 1 \
  || fail "(c): finalize crashed"
out="$(GLUERUN_PLANNER_SESSION=1 gluerun_planner_resume_decide "$canon" "$NODE" codex-run.sh "$wt" "$HEAD2")"
...
for pair in "SID-F1:$RENDERED_A_SHA" "SID-F2:$RENDERED_B_SHA"; do
  ...
  gluerun_ctx_planner_session_finalize "$NODE" 0 "TASK-0010" "RUN-$sid" \
...
GLUERUN_PLANNER_TEMPLATE="$missing_tpl" gluerun_ctx_planner_session_finalize "$NODE" 0 \
...
gluerun_ctx_planner_session_finalize "$NODE" 0 "TASK-0010" "RUN-OFF" \
...
before="$(gluerun_sha256_file "$canon")"
gluerun_ctx_planner_session_finalize "$NODE" 1 "TASK-0010" "RUN-RC" \
...
gluerun_ctx_planner_session_finalize "$NODE" "notanumber" "TASK-0010" "RUN-NAN" \
