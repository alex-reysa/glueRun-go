...
run_lib() {
  ...
  SINGULAR_RUNS_DIR="$tmp/state/runs" \
...
make_evidence() {
  ...
  SINGULAR_ROOT="$root" SINGULAR_STATE_DIR="$state" SINGULAR_RUNS_DIR="$state/runs" \
    bash -c 'source "$1"; singular_runner_result_write codex "$2" planner planner-core "$3" 1 "$4" "" ""' \
...
run_lib singular_cycle_limit_window_evidence_json >/dev/null 2>&1 \
...
ev="$(run_lib singular_cycle_limit_window_evidence_json)" \
...
if run_lib "singular_planner_backoff_set quota RUN-X node-x ''" 2>/dev/null; then
  fail "quota backoff without structured evidence must be refused"
fi
if run_lib "singular_planner_backoff_set quota RUN-X node-x '$tmp/state/runs/RUN-noise/worker-codex.log'" 2>/dev/null; then
...
run_lib "singular_planner_backoff_set quota RUN-X node-x '$evidence'" \
...
out="$(run_lib singular_planner_backoff_clear)"
...
make_evidence "$repo" "$repo/.singular-state" RUN-cycle >/dev/null
...
auto_env() {
  ...
  SINGULAR_TARGET_BRANCH=agent/integration \
  SINGULAR_RECONCILE_SCRIPT="$repo/stub-reconcile.sh" \
  SINGULAR_PUSH=0 \
  "$@"
}
...
[[ ! -f "$repo/.singular-state/planner-backoff.json" ]] \
  || fail "import rejections armed quota backoff"

rm -f "$repo/.singular-state/circuit.json"
touch "$repo/.singular-state/runs/RUN-cycle/runner-result.json"
write_stub 1 0
out="$(auto_env bash "$ENGINE_HOME/engine/autonomate.sh" --once 2>&1)" || true
assert_contains "$out" "armed quota backoff" "dispatch failure + evidence should arm"
[[ -f "$repo/.singular-state/planner-backoff.json" ]] || fail "backoff expected"
assert_contains "$(cat "$repo/.singular-state/planner-backoff.json")" \
  'runner-result.json' "backoff carries result evidence"

rm -f "$repo/.singular-state/planner-backoff.json" "$repo/.singular-state/circuit.json"
touch "$repo/.singular-state/runs/RUN-cycle/runner-result.json"
out="$(SINGULAR_DISABLE_LIMIT_SLEEPTHROUGH=1 auto_env bash "$ENGINE_HOME/engine/autonomate.sh" --once 2>&1)" || true
