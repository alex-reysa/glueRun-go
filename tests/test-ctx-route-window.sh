...
# gluerun_ctx_route_window_gate <role> <session-transcript-path> — the first of
...
#     (byte count / GLUERUN_SESSION_WINDOW_CHARS_PER_TOKEN, floored). The estimate
#     is compared against GLUERUN_SESSION_WINDOW_MAX_PCT of the window size
#     GLUERUN_SESSION_WINDOW_TOKENS.
...
[[ "$(type -t gluerun_ctx_route_window_gate)" == "function" ]] \
  || fail "gluerun_ctx_route_window_gate not defined by $CTX_W"
...
export GLUERUN_SESSION_WINDOW_CHARS_PER_TOKEN=4
# Small window so the fixtures are tiny: threshold = 1000 * 70% = 700 tokens =
# 2800 bytes at 4 bytes/token.
export GLUERUN_SESSION_WINDOW_TOKENS=1000
export GLUERUN_SESSION_WINDOW_MAX_PCT=70

# --- At threshold -> pass ----------------------------------------------------
t="$tmp/at.jsonl"; mk_transcript "$t" 2800   # est = 700 == threshold
out="$(gluerun_ctx_route_window_gate "$ROLE" "$t")"
...
pass "gate: estimate at/under GLUERUN_SESSION_WINDOW_MAX_PCT of window -> pass"
...
out="$(env -u GLUERUN_SESSION_WINDOW_TOKENS -u GLUERUN_SESSION_WINDOW_MAX_PCT \
       -u GLUERUN_SESSION_WINDOW_CHARS_PER_TOKEN bash -c '
         source "'"$LIB"'"; source "'"$CTX_W"'"
         gluerun_ctx_route_window_gate '"$ROLE"' "'"$t"'"')"
assert_eq "$out" "pass" "defaults: tiny transcript passes with default window"
pass "gate: GLUERUN_SESSION_WINDOW_TOKENS / _MAX_PCT default to working values"
...
rc=0
lines="$(gluerun_ctx_route_window_gate "$ROLE" "$tmp/over1.jsonl")" || rc=$?
assert_eq "$rc" "0" "gate exit code is 0 on refuse"
[[ "$(printf '%s\n' "$lines" | wc -l | tr -d ' ')" == "1" ]] || fail "gate printed more than one line"
rc=0
lines="$(gluerun_ctx_route_window_gate "$ROLE" "$tmp/at.jsonl")" || rc=$?
assert_eq "$rc" "0" "gate exit code is 0 on pass"
[[ "$(printf '%s\n' "$lines" | wc -l | tr -d ' ')" == "1" ]] || fail "gate printed more than one line"
rc=0
gluerun_ctx_route_window_gate "$ROLE" "$tmp/does-not-exist.jsonl" >/dev/null || rc=$?
