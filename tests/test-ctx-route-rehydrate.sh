...
# lineage-continuation steps, and only behind GLUERUN_REHYDRATE=1, this leaf
...
#   - OFF-parity: GLUERUN_REHYDRATE unset (or =0) -> ALWAYS `fresh <reason>`,
...
#   - ON upgrade: GLUERUN_REHYDRATE=1, a non-pinned lineage-continuation step, and
#     a manifest with >=1 surviving source -> `rehydrate <reason>` for each reason.
#   - Independence guard (evidence invariance): GLUERUN_REHYDRATE=1 with a non-empty
#     manifest still yields `fresh <reason>` for step=final-audit and
#     step=paired-audit; it NEVER rehydrates a pinned step, reusing
#     gluerun_ctx_route_independence_admit so the independence set stays single-sourced.
#   - Empty-packet guard: GLUERUN_REHYDRATE=1, a non-pinned step, and a manifest
...
#     `rehydrate` strategy stays tainted per gluerun_ctx_route_strategy_tainted).
...
[[ "$(type -t gluerun_ctx_route_rehydrate_decide)" == "function" ]] \
  || fail "gluerun_ctx_route_rehydrate_decide not defined by $CTX_R"

# Manifests: one carrying a surviving source, one whose sources array is empty.
FULL='{"schema":"gluerun.orchestration.ctx-rehydrate-manifest.v0","sources":[{"id":"task-packet","sha256":"deadbeef"}]}'
EMPTY='{"schema":"gluerun.orchestration.ctx-rehydrate-manifest.v0","sources":[]}'

REASONS=(session-lease window-pressure diff-volume)

# --- OFF-parity: GLUERUN_REHYDRATE unset/0 -> always `fresh <reason>` ----------
unset GLUERUN_REHYDRATE
for reason in "${REASONS[@]}"; do
  assert_eq "$(gluerun_ctx_route_rehydrate_decide "$reason" implementer retry-resume "$FULL")" \
  ...
  assert_eq "$(gluerun_ctx_route_rehydrate_decide "$reason" worker retry "$FULL")" \
    "fresh $reason" "OFF (unset) worker retry full manifest -> fresh [$reason]"
done
export GLUERUN_REHYDRATE=0
for reason in "${REASONS[@]}"; do
  assert_eq "$(gluerun_ctx_route_rehydrate_decide "$reason" implementer retry-resume "$FULL")" \
    "fresh $reason" "OFF (=0) non-pinned full manifest -> fresh [$reason]"
  assert_eq "$(gluerun_ctx_route_rehydrate_decide "$reason" implementer retry-resume "$EMPTY")" \
    "fresh $reason" "OFF (=0) empty manifest -> fresh [$reason]"
  assert_eq "$(gluerun_ctx_route_rehydrate_decide "$reason" auditor final-audit "$FULL")" \
    "fresh $reason" "OFF (=0) pinned step -> fresh [$reason]"
done
pass "OFF-parity: GLUERUN_REHYDRATE unset/0 yields fresh <reason> verbatim for all reasons"
...
for reason in "${REASONS[@]}"; do
  ...
  assert_eq "$(gluerun_ctx_route_rehydrate_decide "$reason" planner revise "$FULL")" \
...
for reason in "${REASONS[@]}"; do
  ...
  assert_eq "$(gluerun_ctx_route_rehydrate_decide "$reason" auditor paired-audit "$FULL")" \
...
# gluerun_ctx_route_independence_admit rehydrate <role> <step> must NOT admit.
for step in final-audit paired-audit; do
  verdict="$(gluerun_ctx_route_independence_admit rehydrate auditor "$step")"
  [[ "$verdict" != "admit" ]] \
...
pass "no independence conferred: rehydrate is tainted per gluerun_ctx_route_strategy_tainted"
...
for row in "${rows[@]}"; do
  ...
  rc=0; line="$(gluerun_ctx_route_rehydrate_decide "$r" "$ro" "$st" "$mf")" || rc=$?
