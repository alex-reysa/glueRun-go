...
# singular_ctx_route_diff_gate <role> <worktree> <head-sha-at-create> <lineage-head>
...
# SINGULAR_SESSION_DIFF_MAX_LINES.
#
...
#   - Additive documented knob SINGULAR_SESSION_DIFF_MAX_LINES with a default;
...
[[ "$(type -t singular_ctx_route_diff_gate)" == "function" ]] \
  || fail "singular_ctx_route_diff_gate not defined by $CTX_D"

ROLE="implementer"
export SINGULAR_SESSION_DIFF_MAX_LINES=10
...
# SINGULAR_SESSION_DIFF_MAX_LINES (used only by the default-knob assertion below).
...
out="$(singular_ctx_route_diff_gate "$ROLE" "$wt" "$HEAD0" "$HEAD1" -- src)"
...
out="$(singular_ctx_route_diff_gate "$ROLE" "$wt" "$HEAD0" "$HEAD1")"
...
out="$(singular_ctx_route_diff_gate "$ROLE" "$wt" "$HEAD0" "$HEAD2" -- src)"
assert_eq "$out" "refuse diff-volume" "scoped churn over threshold refuses"
pass "gate: churn in role-relevant paths over SINGULAR_SESSION_DIFF_MAX_LINES -> refuse diff-volume"

# --- Fail closed: empty / absent shas ----------------------------------------
out="$(singular_ctx_route_diff_gate "$ROLE" "$wt" "" "$HEAD1" -- src)"
assert_eq "$out" "refuse diff-volume" "empty head-sha-at-create fails closed"
out="$(singular_ctx_route_diff_gate "$ROLE" "$wt" "$HEAD0" "" -- src)"
assert_eq "$out" "refuse diff-volume" "empty lineage head fails closed"
out="$(singular_ctx_route_diff_gate "$ROLE" "$wt" "" "" -- src)"
...
out="$(singular_ctx_route_diff_gate "$ROLE" "$wt" "deadbeefdeadbeefdeadbeefdeadbeefdeadbeef" "$HEAD1" -- src)"
assert_eq "$out" "refuse diff-volume" "nonexistent sha fails closed"
out="$(singular_ctx_route_diff_gate "$ROLE" "$tmp/not-a-repo" "$HEAD0" "$HEAD1" -- src)"
...
out="$(env -u SINGULAR_SESSION_DIFF_MAX_LINES bash -c '
         source "'"$LIB"'"; source "'"$CTX_D"'"
         singular_ctx_route_diff_gate '"$ROLE"' "'"$wt"'" "'"$HEAD0"'" "'"$HEAD3"'"')"
...
out="$(env -u SINGULAR_SESSION_DIFF_MAX_LINES bash -c '
         source "'"$LIB"'"; source "'"$CTX_D"'"
         singular_ctx_route_diff_gate '"$ROLE"' "'"$wt"'" "'"$HEAD0"'" "'"$HEAD1"'" -- src')"
assert_eq "$out" "pass" "default threshold passes a 3-line scoped churn"
pass "gate: SINGULAR_SESSION_DIFF_MAX_LINES defaults to a finite working value"
...
for args in \
  ...
  out="$(singular_ctx_route_diff_gate $args)"
  case "$out" in
...
rc=0
lines="$(singular_ctx_route_diff_gate "$ROLE" "$wt" "$HEAD0" "$HEAD2" -- src)" || rc=$?
assert_eq "$rc" "0" "gate exit code is 0 on refuse"
[[ "$(printf '%s\n' "$lines" | wc -l | tr -d ' ')" == "1" ]] || fail "gate printed more than one line"
rc=0
lines="$(singular_ctx_route_diff_gate "$ROLE" "$wt" "$HEAD0" "$HEAD1" -- src)" || rc=$?
assert_eq "$rc" "0" "gate exit code is 0 on pass"
[[ "$(printf '%s\n' "$lines" | wc -l | tr -d ' ')" == "1" ]] || fail "gate printed more than one line"
rc=0
singular_ctx_route_diff_gate "$ROLE" "$wt" "" "$HEAD1" -- src >/dev/null || rc=$?
