...
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp" "$SINGULAR_ROOT"' EXIT
root="$tmp/repo"
...
export SINGULAR_TASKBATCH_SCHEMA="$root/schemas/orchestration/task-batch.v0.schema.json"
...
body='{"schema":"singular.orchestration.task-batch.v0","tasks":[]}'
[[ -n "$out" ]] && printf '%s\n' "$body" >"$out"
printf '%s\n' "$body"
SH
chmod +x "$stub"
...
sdir="$SINGULAR_STATE_DIR/runs/RUN-E/l1-staging/S0.base"
rc=0
out="$(SINGULAR_CODEX_RUNNER="$stub" "$SCRIPT_DIR/generate-tasks.sh" --node S0.base --stage-dir "$sdir" --count 2 2>&1)" || rc=$?
...
events="$(cat "$SINGULAR_EVENTS_FILE" 2>/dev/null || true; cat "$sdir/planner-events.ndjson" 2>/dev/null || true)"
...
[[ -f "$SINGULAR_STATE_DIR/planner-backoff.json" ]] && fail "empty batch must not arm a backoff"

# 2. Importer over the NO-TASKS node: lease released, no rejection.
singular_l1_lease_write S0.base storage S0 substrate active RUN-E "$(git -C "$root" rev-parse target)" target
out="$(singular_l1_import_staged RUN-E S0.base 2>&1)"
...
[[ -f "$SINGULAR_L1_LEASES_DIR/S0.base.json" ]] && fail "node lease must be released"
assert_contains "$(cat "$SINGULAR_EVENTS_FILE")" '"type":"origin.l1_no_tasks"' "importer event"
...
body='{"schema":"singular.orchestration.task-batch.v0","tasks":[{"bogus":true}]}'
[[ -n "$out" ]] && printf '%s\n' "$body" >"$out"
printf '%s\n' "$body"
SH
chmod +x "$stub"
rc=0
out="$(SINGULAR_CODEX_RUNNER="$stub" "$SCRIPT_DIR/generate-tasks.sh" --node S0.base --stage-dir "$SINGULAR_STATE_DIR/runs/RUN-F/l1-staging/S0.base" --count 2 2>&1)" || rc=$?
