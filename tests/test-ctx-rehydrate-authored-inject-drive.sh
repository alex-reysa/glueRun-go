...
# GLUERUN_REHYDRATE=1), AFTER the durable rehydration packet (TASK-0057) is
# appended, the driver ALSO appends the eligible authored-knowledge entries —
# equal to `gluerun_ctx_rehydrate_authored_config_render implement` — under a
# reference-only / NOT-authoritative wrapper. The append is delegated entirely
# into the integrated config-gated render; with GLUERUN_CTX_MANIFEST unset (or no
...
#               gate (GLUERUN_SESSION_WINDOW_MAX_PCT=0) refuses it. Behind
#               GLUERUN_REHYDRATE=1 the refusal upgrades to `rehydrate`. The worker
...
workroot="$(mktemp -d "${TMPDIR:-/tmp}/gluerun-rehydrate-authored-inject-drive.XXXXXX")"
...
# origin root's default GLUERUN_JSON_CONFIG_FILE path; a RELATIVE contextManifest
...
expected_authored() {
  GLUERUN_CTX_MANIFEST=1 GLUERUN_JSON_CONFIG_FILE="$drv_root/gluerun.config.json" \
...
# ---------------------------------------------------------------------------
# (ON) rehydrate + GLUERUN_CTX_MANIFEST=1 + configured contextManifest: the
# authored section is appended AFTER the durable packet, under a reference-only
# header, and equals gluerun_ctx_rehydrate_authored_config_render implement.
# ---------------------------------------------------------------------------
reset_state
...
pass "(OFF-parity manifest) GLUERUN_CTX_MANIFEST unset: durable-only injection, no authored section"
...
pass "(OFF-parity rehydrate) GLUERUN_REHYDRATE unset: no durable and no authored injection"
