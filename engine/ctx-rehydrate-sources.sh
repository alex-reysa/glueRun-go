...
# (singular_ctx_rehydrate_packet / singular_ctx_rehydrate_manifest): the emitted
...
# and singular_ctx_artifact_scan_paths scans). This is a curated, class-tagged
...
# mirroring singular_ctx_artifact_scan_paths; a missing artifact contributes
...
# compose singular_ctx_artifact_exclude, so a single quarantine authority is
...
# `rehydrate` strategy remains tainted per singular_ctx_route_strategy_tainted.
