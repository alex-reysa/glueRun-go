...
# guard, and the `singular experiment-report` CLI all attribute escape-rate / cost
...
# engine's own SINGULAR_* continuity knobs). It creates, moves, or mutates NOTHING
...
#
# Knob-state definitions (the canonical SINGULAR_* names the engine itself defines,
# each with the engine's own documented default and active interpretation):
#   * truthy : set, non-empty, and not "0"  — SINGULAR_CTX_PACKET, SINGULAR_CTX_ARTIFACT_SCAN
#              (mirrors l1-drive.sh assumptions_ctx_enabled / the artifact-scan gate).
#   * flag1  : exactly "1"                   — SINGULAR_CTX_ROUTING, SINGULAR_REHYDRATE,
#              SINGULAR_CTX_MANIFEST (mirrors ctx-route.sh / ctx-rehydrate-authored-config.sh).
#   * pct    : a positive-integer percentage — SINGULAR_PAIRED_AUDIT_PCT,
...
#
# Public entry points:
#   singular_ctx_experiment_armstate_read <knob>
...
#     singular.orchestration.ctx-experiment-armstate.v0, composing the snapshot
...
_singular_ctx_experiment_armstate_py() {
  cat <<'PY'
import json, os, re
...
KNOBS = (
    ...
    ("SINGULAR_REHYDRATE", "0", "flag1"),
    ("SINGULAR_PAIRED_AUDIT_PCT", "0", "pct"),
    ("SINGULAR_CRITIC_RECHECK_PCT", "0", "pct"),
    ("SINGULAR_CTX_ARTIFACT_SCAN", "0", "truthy"),
    ("SINGULAR_CTX_MANIFEST", "0", "flag1"),
)
...
singular_ctx_experiment_armstate_read() {
  local knob="${1:-}"
...
knobs = snapshot()
artifact = {
    "schema": "singular.orchestration.ctx-experiment-armstate.v0",
    "knobs": knobs,
