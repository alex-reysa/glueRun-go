...
# inner layer with the task/commit/audit node families. A later `gluerun graph
# rebuild` entry point (behind GLUERUN_CTX_GRAPH, default 0) partitions the stream
# by `kind` and hands the node stream to the integrated corpus writer
# gluerun_graph_write_corpus (engine/ctx-graph-corpus.sh).
#
# Evidence class is fail-closed and delegated to gluerun_graph_emit_node: `commit`
...
#
# Public functions:
#   gluerun_graph_project_task <taskFilePath>
...
#   gluerun_graph_project_commits <eventsPath>
...
#   gluerun_graph_project_paired_audits <pairedAuditRecordPath>
#       -> reads a gluerun.orchestration.paired-audit.v0 record (runId, taskId,
...
') || return 1
  ...
  local ident
  ident="$(gluerun_graph_identity task "$task_id")"
  gluerun_graph_emit_node task "$ident" "$task_path" "$content"
}
...
gluerun_graph_project_commits() {
  ...
  while IFS=$'\037' read -r head_sha content_b64; do
    ...
    content="$(printf '%s' "$content_b64" | base64 --decode)"
    ident="$(gluerun_graph_identity commit "$head_sha")"
    gluerun_graph_emit_node commit "$ident" "$events_path" "$content"
...
# authoritative even for a verdict) from a gluerun.orchestration.paired-audit.v0
...
gluerun_graph_project_paired_audits() {
  local record_path="$1"
...
') || return 1
  ...
  content="$(printf '%s' "$content_b64" | base64 --decode)"
  ident="$(gluerun_graph_identity audit "$run_id")"
  gluerun_graph_emit_node audit "$ident" "$record_path" "$content" "" "$attrs"
}
