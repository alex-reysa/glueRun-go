...
# gluerun_ctx_assumptions_transition is applied at attempt-close by the later driver hook).
#
...
#
# Reads <task-file> STRICTLY READ-ONLY (composing gluerun_ctx_assumptions_seed) and reads
# the prior-ledger JSON argument, then prints a single attempt-open envelope JSON on stdout:
#   {"schema":"gluerun.orchestration.ctx-assumptions-run.v0",
...
gluerun_ctx_assumptions_assemble() {
  ...
  seed="$(gluerun_ctx_assumptions_seed "$task_file")"
  ledger="$(gluerun_ctx_assumptions_carry "$prior" "$seed")"
  fix="$(gluerun_ctx_assumptions_fix_section "$ledger")"
  audit="$(gluerun_ctx_assumptions_audit_section "$ledger")"
  gluerun_ctx_assumptions_assemble_py "$ledger" "$fix" "$audit"
}
...
# effects.
gluerun_ctx_assumptions_assemble_py() {
  python3 - "$1" "$2" "$3" <<'PY'
import json, sys
