...
# by delegating to singular_ctx_experiment_summary_json, or from a supplied bundle
...
#   3. singular_ctx_experiment_delta_json [runs_dir] [events_file] [metrics_file]
#      — public entry: obtain the summary bundle and emit ONE deterministic
#      sorted-key JSON object under singular.orchestration.ctx-experiment-delta.v0.
...
_SINGULAR_CTX_EXPERIMENT_DELTA_PY="$(cat <<'PY'
def _num(obj, path):
    cur = obj
    for k in path:
...
singular_ctx_experiment_delta_record() {
  local arm_a="${1:-}" arm_b="${2:-}"
  shift 2 2>/dev/null || true
  SINGULAR_DELTA_A="$arm_a" SINGULAR_DELTA_B="$arm_b" python3 - "$@" <<PY || true
$_SINGULAR_CTX_EXPERIMENT_DELTA_PY
import json, os, sys
...
arm_a = load(os.environ.get("SINGULAR_DELTA_A", "{}"))
arm_b = load(os.environ.get("SINGULAR_DELTA_B", "{}"))
...
singular_ctx_experiment_delta_metrics() {
  local bundle="${1:-}"
  SINGULAR_DELTA_BUNDLE="$bundle" python3 - <<PY || true
$_SINGULAR_CTX_EXPERIMENT_DELTA_PY
import json, os, sys
...
bundle = load(os.environ.get("SINGULAR_DELTA_BUNDLE", "{}"))
...
# bundle source file via SINGULAR_CTX_EXPERIMENT_DELTA_BUNDLE, else by threading
# the explicit corpus args through singular_ctx_experiment_summary_json), compute
# the per-metric treatment-effect deltas, and emit ONE deterministic sorted-key
# JSON object conforming to singular.orchestration.ctx-experiment-delta.v0.
# Fail-safe / always exit 0.
singular_ctx_experiment_delta_json() {
  ...
  local bundle bundle_src="${SINGULAR_CTX_EXPERIMENT_DELTA_BUNDLE:-}"
  ...
  local deltas
  deltas="$(singular_ctx_experiment_delta_metrics "$bundle")"

  SINGULAR_DELTA_DELTAS="$deltas" python3 - <<'PY' || true
import json, os, sys
...
artifact = {
    ...
    "deltas": load(os.environ.get("SINGULAR_DELTA_DELTAS", "{}")),
}
