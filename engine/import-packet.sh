...
mkdir -p "$run_dir"
if ! gluerun_packet_module_guard "$packet" "$task_file" "$workspace" "$run_dir" >"$guard_log" 2>&1; then
  cat "$guard_log" >&2
  gluerun_append_event "packet.import_rejected" "import rejected: module packet guard" \
...
actual_head="$(git -C "$GLUERUN_ROOT" rev-parse --short "$branch")"
...
# Set GLUERUN_REQUIRE_AUDIT=0 only for explicit, documented bootstrap exceptions.
...
if [[ "$require_audit" == "1" ]]; then
  if [[ ! -f "$audit_record" ]]; then
    if gluerun_packet_has_accept_waiver "$packet"; then
      ...
      gluerun_append_event "packet.import_rejected" "import rejected: missing auditor verdict" \
        "{\"taskId\":\"$task_id\",\"runId\":\"$run_id\",\"reason\":\"missing-audit\"}"
      exit 2
    fi
  else
    verdict="$(gluerun_json_field "$audit_record" verdict 2>/dev/null || echo unknown)"
    acceptance_mode="$(gluerun_packet_acceptance_mode "$packet" "$audit_record" 2>/dev/null || true)"
    if [[ -z "$acceptance_mode" ]]; then
      ...
      gluerun_append_event "packet.import_rejected" "import rejected: auditor not accepted" \
...
dest_dir="$GLUERUN_ORCH_DIR/packets/imported/$task_id"
...
gluerun_append_event "packet.imported" "state packet imported" \
