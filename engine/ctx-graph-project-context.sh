...
# inner layer with the assumption/capsule node families. A later `gluerun graph
...
#
# Evidence class is fail-closed and delegated to gluerun_graph_emit_node: both
...
#
# Public functions:
#   gluerun_graph_project_assumptions <taskFilePath>
...
#   gluerun_graph_project_capsules <capsuleRecordPath>
...
gluerun_graph_project_assumptions() {
  local task_path="$1"
  local task_id
  task_id="$(GLUERUN_GP_PATH="$task_path" python3 -c '
import os, re, sys
...
')" || return 1
  ...
  while IFS=$'\037' read -r key status content_b64; do
    ...
    ident="$(gluerun_graph_identity assumption "$task_id" "$key")"
    attrs="$(GLUERUN_GP_STATUS="$status" python3 -c '
import json, os
print(json.dumps({"status": os.environ["GLUERUN_GP_STATUS"]}, separators=(",", ":")))')"
    gluerun_graph_emit_node assumption "$ident" "$task_path" "$content" "" "$attrs"
  done < <(GLUERUN_GP_PATH="$task_path" python3 -c '
...
# file.
gluerun_graph_project_capsules() {
  local record_path="$1"
...
') || return 1
  ...
  content="$(printf '%s' "$content_b64" | base64 --decode)"
  ident="$(gluerun_graph_identity capsule "$run_id" "$role")"
  gluerun_graph_emit_node capsule "$ident" "$record_path" "$content" "" "$attrs"
}
