...
# default-OFF GLUERUN_CRITIC_RECHECK_PCT; TASK-0028
...
# decider gluerun_plan_recritic_resume_decide (TASK-0025), exactly as the codebase
...
# (not revised candidates), gated by GLUERUN_CRITIC_RECHECK_PCT (not
# GLUERUN_PLAN_RECRITIC_RESUME), destined for the l1-drive.sh post-acceptance hook
# (not the in-loop l1-plan-node.sh revision loop) — so it advances the stage rather
# than duplicating TASK-0025. It reuses the gluerun.orchestration.session-meta.v0
...
# GLUERUN_CRITIC_RECHECK_PCT. Events land only in the pinned GLUERUN_EVENTS_FILE.
#
...
#   1. disabled              GLUERUN_CRITIC_RECHECK_PCT unset / 0 / non-numeric (default 0 = OFF)
...
gluerun_ctx_critic_recheck_resume_template_path() {
...
gluerun_ctx_critic_recheck_resume_lease_path() {
  local node="$1"
...
gluerun_ctx_critic_recheck_resume_lease_live() {
  local lease_path="$1"
...
#   gluerun_ctx_critic_recheck_resume_decide <critic_session_meta> <node> \
...
gluerun_ctx_critic_recheck_resume_decide() {
  ...
  # GLUERUN_CRITIC_RECHECK_PCT is unset, "0", or non-numeric garbage; no session is
  # ever resumed unless the percentage knob is a positive integer.
  local pct="${GLUERUN_CRITIC_RECHECK_PCT:-0}"
...
PY
  ...
  tpl_path="$(gluerun_ctx_critic_recheck_resume_template_path)"
...
PY
  ...
  # concurrently.
  local lease_path
  lease_path="$(gluerun_ctx_critic_recheck_resume_lease_path "$node")"
  if gluerun_ctx_critic_recheck_resume_lease_live "$lease_path"; then
    printf 'fresh leased\n'; return 0
  fi
...
# event through gluerun_append_event carrying node, runId, `taskId` (the accepted
...
#   gluerun_ctx_critic_recheck_record_strategy <node> <run_id> <task_id> \
#                                              <strategy> <reason> [session_id]
gluerun_ctx_critic_recheck_record_strategy() {
