#!/usr/bin/env bash
# ctx-critic-recheck-resume.sh — the POST-ACCEPTANCE-RECHECK resume authority
# brick. Stage-file deliverable of the executable DAG node `critic-carryover`
# (stage S3-plan-revision, area plancritic, layer engine_runtime, kind runtime):
# the fail-closed RESUME AUTHORITY that decides whether the post-acceptance recheck
# may RESUME the persisted plan-critic session READ-ONLY over the ACCEPTED diff,
# enforcing the skeptic role gate (this session may never be resumed as
# planner/implementer), plus the record-only strategy provenance emitter.
#
# The two ends of the recheck are already integrated: TASK-0027
# (engine/ctx-critic-recheck.sh) decides WHETHER an accepted task is sampled behind
# default-OFF SINGULAR_CRITIC_RECHECK_PCT; TASK-0028
# (engine/ctx-critic-recheck-classify.sh) classifies the resumed critic's
# self-report into per-finding addressed|survives|obsolete and emits
# ctx.critic_recheck. What was still missing — the OUT OF SCOPE follow-up named in
# both files' headers — is the authority that SAFELY RESUMES the plan-critic session
# for that recheck. This file supplies it.
#
# It is the post-acceptance-recheck sibling of the integrated in-loop re-critique
# decider singular_plan_recritic_resume_decide (TASK-0025), exactly as the codebase
# already carries parallel resume deciders for the planner (TASK-0009) and the
# critic re-critique (TASK-0025). It is a DIFFERENT engagement — the ACCEPTED diff
# (not revised candidates), gated by SINGULAR_CRITIC_RECHECK_PCT (not
# SINGULAR_PLAN_RECRITIC_RESUME), destined for the l1-drive.sh post-acceptance hook
# (not the in-loop l1-plan-node.sh revision loop) — so it advances the stage rather
# than duplicating TASK-0025. It reuses the singular.orchestration.session-meta.v0
# shape the plan-critic driver FINALIZES at
# <state-dir>/sessions/plan-critic/<node>.json (role plan-critic).
#
...
# engine/ctx-critic-recheck.sh / ctx-critic-recheck-classify.sh / ctx-plan-recritic-
# resume.sh): no events, no state writes. It adds NO driver-file hook, invokes NO
# runner, and never owns engine/lib.sh.
#
...
# lets a resumed (tainted) critic session satisfy an independence-required step,
# never makes the fresh implementation auditor bypassable, and never changes any
# accept/reject outcome. The whole recheck stays default-OFF behind
# SINGULAR_CRITIC_RECHECK_PCT. Events land only in the pinned SINGULAR_EVENTS_FILE.
#
# The read-only critic-resume RUNNER that consults this decider + the sampling gate
# (TASK-0027) and feeds the classifier/recorder (TASK-0028), and the l1-drive.sh
# post-acceptance hook, are the sanctioned follow-up slices of `critic-carryover`
# and are OUT OF SCOPE here.
#
# Gate order (first failure wins):
#   1. disabled              SINGULAR_CRITIC_RECHECK_PCT unset / 0 / non-numeric (default 0 = OFF)
...
#   6. head-rewritten        meta.headShaAtCreate NOT an ancestor of accepted-diff head
...
#  11. leased                a LIVE plan-critic session-lease held
...
singular_ctx_critic_recheck_resume_template_path() {
  ...
  # (tests/test-plan-critique-schema.sh) and tests/test-engine-clean.sh assert NO
  # engine path carries the literal critic prompt filename, and
  # engine/ctx-plan-critic.sh finalizes the sha against this same part-assembled
  # path. Runtime resolution is unaffected.
...
singular_ctx_critic_recheck_resume_lease_path() {
  local node="$1"
...
singular_ctx_critic_recheck_resume_lease_live() {
  local lease_path="$1"
...
# Decide whether the post-acceptance recheck may RESUME the recorded critic session
# READ-ONLY over the accepted diff. Prints EXACTLY one line: `resume <sessionId>` or
# `fresh <reason>`. Never exits non-zero. Any ambiguity resolves to `fresh
# <reason>`, NEVER to resume. Pure decision — appends no event, mutates nothing.
#   singular_ctx_critic_recheck_resume_decide <critic_session_meta> <node> \
#       <runner_basename> <worktree> <lineage_head>
singular_ctx_critic_recheck_resume_decide() {
  local meta_path="$1" node="$2" runner="$3" worktree="$4" lineage_head="$5"

  # Gate 1: enable knob (default 0 = OFF). The whole recheck feature is OFF when
  # SINGULAR_CRITIC_RECHECK_PCT is unset, "0", or non-numeric garbage; no session is
  # ever resumed unless the percentage knob is a positive integer.
  local pct="${SINGULAR_CRITIC_RECHECK_PCT:-0}"
...
PY
  ...
  # role is exactly "plan-critic"; a planner/implementer-role session may NEVER be
  # resumed for a recheck.
  ...
  # Gate 6: node-lineage — headShaAtCreate must be an ancestor of the accepted-diff
  # lineage head. Fail closed: empty stored head, empty lineage head, or
  ...
  tpl_path="$(singular_ctx_critic_recheck_resume_template_path)"
...
PY
  ...
  # concurrently.
  local lease_path
  lease_path="$(singular_ctx_critic_recheck_resume_lease_path "$node")"
  if singular_ctx_critic_recheck_resume_lease_live "$lease_path"; then
    printf 'fresh leased\n'; return 0
  fi
...
# event through singular_append_event carrying node, runId, `taskId` (the accepted
# task being rechecked), strategy (`resume` or `fresh`), the exact `reason`, and
# `sessionId` on resume — so a recheck that re-enters the same critic session is
# observable to ctx-metrics.sh. No lease change, no runner, no outcome mutation.
#   singular_ctx_critic_recheck_record_strategy <node> <run_id> <task_id> \
#                                              <strategy> <reason> [session_id]
singular_ctx_critic_recheck_record_strategy() {
  local node="${1:-}" run_id="${2:-}" task_id="${3:-}" \
        strategy="${4:-}" reason="${5:-}" session_id="${6:-}"

  local event_json message
  event_json="$(python3 - "$node" "$run_id" "$task_id" "$strategy" "$reason" "$session_id" <<'PY'
import json, sys
node, run_id, task_id, strategy, reason, session_id = sys.argv[1:7]
data = {
    "node": node,
    "runId": run_id,
    "taskId": task_id,
...
PY
  ...
  if [[ "$strategy" == "resume" ]]; then
    message="critic recheck session resume strategy selected"
  else
    message="critic recheck fresh-run strategy selected"
  fi
