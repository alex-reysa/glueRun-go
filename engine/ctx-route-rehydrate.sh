...
# lineage-continuation steps, and only behind SINGULAR_REHYDRATE=1, this decision
...
#
#   - OFF-parity: when SINGULAR_REHYDRATE is unset or != 1, it ALWAYS prints
...
#   - Otherwise (SINGULAR_REHYDRATE=1, a non-pinned lineage-continuation step, and
#     a manifest with at least one surviving source) it prints `rehydrate
#     <refusal-reason>`.
#
# SINGULAR_REHYDRATE (default 0) gates ONLY this decision's upgrade; the leaf
...
singular_ctx_route_rehydrate_decide() {
  ...
  # OFF-parity: only SINGULAR_REHYDRATE=1 arms the upgrade; anything else (unset,
  ...
  if [[ "$(singular_ctx_route_independence_admit rehydrate "$role" "$step")" != "admit" ]]; then
    printf 'fresh %s\n' "$reason"; return 0
  fi
  ...
  if ! _singular_ctx_route_rehydrate_has_source "$manifest"; then
    printf 'fresh %s\n' "$reason"; return 0
  fi
...
_singular_ctx_route_rehydrate_has_source() {
  local manifest="$1"
  [[ -n "$manifest" ]] || return 1
