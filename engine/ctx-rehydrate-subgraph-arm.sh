...
# (gluerun_ctx_rehydrate_subgraph_select), resolution
# (gluerun_ctx_rehydrate_subgraph_sources), and the order-preserving assembler
# (gluerun_ctx_rehydrate_subgraph_assemble) — but nothing yet CHOOSES it per arm;
...
#
# Knob: GLUERUN_CTX_SUBGRAPH_REHYDRATE (default 0). Only "1" can enable subgraph
...
# strategy stays tainted per gluerun_ctx_route_strategy_tainted regardless of
...
#       (1) GLUERUN_CTX_SUBGRAPH_REHYDRATE=1 (the dedicated feature knob);
#       (2) the task's deterministic A/B arm — via the integrated
#           gluerun_ctx_ab_arm_for (engine/ctx-ab.sh) — is the designated
...
#           ${GLUERUN_CTX_GRAPH_DIR:-.gluerun-state/graph}) exists and is
#           non-empty — a present, non-empty <graphDir>/nodes.jsonl — so
#           gluerun_ctx_rehydrate_subgraph_assemble could actually render a
#           packet.
#     Any failing precondition yields `flat`.

gluerun_ctx_rehydrate_subgraph_arm_mode() {
  # The designated treatment arm. gluerun_ctx_ab_arm_for maps each id to {A,B};
  ...
  if [[ "${GLUERUN_CTX_SUBGRAPH_REHYDRATE:-0}" != "1" ]]; then
    printf 'flat\n'
    return 0
  fi
  ...
  if [[ "$(gluerun_ctx_ab_arm_for "$task_id")" != "$treatment_arm" ]]; then
    printf 'flat\n'
    return 0
  fi
