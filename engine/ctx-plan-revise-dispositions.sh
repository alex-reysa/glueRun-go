...
# lease.
#
# The SINGULAR_PLAN_CRITIQUE-gated resume-vs-fresh planner re-invocation (resuming
# the persisted node session via singular_planner_resume_decide, resume-refused
...
#
# Public entry points:
#   singular_plan_revise_classify <critique_record> <revised_batch>
#     PURE and READ-ONLY. Reads every `f-[0-9a-f]{12}` finding id from the
#     singular.orchestration.plan-critique.v0 record at <critique_record> and the
...
#     Records ONLY: emits provenance through singular_append_event under event type
...
try:
    ...
    if (isinstance(bdoc, dict)
            and bdoc.get("schema") == "singular.orchestration.task-batch.v0"
...
singular_plan_revise_record_dispositions() {
  ...
  local classified
  classified="$(singular_plan_revise_classify "$critique_record" "$revised_batch")"
...
PY
  ...
  singular_append_event "plan.revised" "per-finding revision dispositions recorded" "$event_json"
  return 0
}
