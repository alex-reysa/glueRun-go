...
# plus engine/ctx-graph.sh's gluerun_graph_node_id / gluerun_graph_emit_node /
# gluerun_graph_emit_edge — into the graph-projector's inner layer. A later
# `gluerun graph rebuild` entry point (behind GLUERUN_CTX_GRAPH, default 0)
# partitions the stream by `kind` and hands the node/edge streams to the
# integrated corpus writer gluerun_graph_write_corpus (engine/ctx-graph-corpus.sh).
#
# Public functions:
#   gluerun_graph_identity <type> <key...>
...
#          gluerun_graph_node_id(identity) AGREE across mappers and cross-family
#          edges connect (e.g. an attempt `implements` the same `task` node a
#          gate-result `verifies`).
#   gluerun_graph_project_attempts <attemptsIndexPath>
#       -> reads an attempts/index.json (gluerun.orchestration.attempts-index.v0:
...
#   gluerun_graph_project_gate_results <gateRecordPath>
#       -> reads a gluerun.orchestration.gate-result.v0 record and emits one
...
gluerun_graph_project_attempts() {
  local index_path="$1"
  local run_id task_id
  run_id="$(GLUERUN_GP_PATH="$index_path" python3 -c '
import json, os
d = json.load(open(os.environ["GLUERUN_GP_PATH"]))
print(d["runId"])' )" || return 1
  task_id="$(GLUERUN_GP_PATH="$index_path" python3 -c '
import json, os
d = json.load(open(os.environ["GLUERUN_GP_PATH"]))
print(d["taskId"])' )" || return 1
  ...
  while IFS=$'\t' read -r n content_b64; do
    ...
    attempt_ident="$(gluerun_graph_identity attempt "$run_id" "$n")"
    attempt_node="$(gluerun_graph_node_id "$attempt_ident")"
    gluerun_graph_emit_node attempt "$attempt_ident" "$index_path" "$content"
    gluerun_graph_emit_edge implements "$attempt_node" "$task_node" "$index_path" "$content"
  done < <(GLUERUN_GP_PATH="$index_path" python3 -c '
...
gluerun_graph_project_gate_results() {
  local record_path="$1"
...
') || return 1
  ...
  gate_ident="$(gluerun_graph_identity gate-result "$node" "$decided_by" "$recorded_at")"
  gate_node="$(gluerun_graph_node_id "$gate_ident")"
  target_node="$(gluerun_graph_node_id "$(gluerun_graph_identity task "$node")")"
