...
# graph` call site is a later task. gluerun_graph_rebuild writes ONLY under the
# passed <graphDir> (via gluerun_graph_write_corpus); it reads <stateDir> and
...
# canonical writing to gluerun_graph_write_corpus (engine/ctx-graph-corpus.sh).
...
#
# Public functions:
#   gluerun_graph_partition <nodesOut> <edgesOut>
...
#       gluerun_graph_project_attempts; docs/orchestration/gates/*.gate-result.json
#       via gluerun_graph_project_gate_results; events.ndjson via
#       gluerun_graph_project_plan_versions / gluerun_graph_project_decisions /
#       gluerun_graph_project_commits; runs/*/plan-critique.json via
#       gluerun_graph_project_critique; docs/orchestration/tasks/TASK-*.md via
#       gluerun_graph_project_task; and runs/*/paired-audit.json via
#       gluerun_graph_project_paired_audits — collect every emitted line, partition
#       it, and call gluerun_graph_write_corpus <graphDir> to write the canonical
...
gluerun_graph_partition() {
  local nodes_out="$1" edges_out="$2"
  GLUERUN_GP_NODES="$nodes_out" GLUERUN_GP_EDGES="$edges_out" python3 -c '
import json, os, sys
with open(os.environ["GLUERUN_GP_NODES"], "w", encoding="utf-8") as nf, \
     open(os.environ["GLUERUN_GP_EDGES"], "w", encoding="utf-8") as ef:
...
# with gluerun_graph_partition, and hand the two streams to
# gluerun_graph_write_corpus <graphDir>. Deterministic + loss-free: the writer's
...
gluerun_graph_rebuild() {
  ...
  if [[ -f "$state_dir/events.ndjson" ]]; then
    gluerun_graph_project_plan_versions "$state_dir/events.ndjson" >> "$combined" || rc=1
    gluerun_graph_project_decisions     "$state_dir/events.ndjson" >> "$combined" || rc=1
    gluerun_graph_project_commits       "$state_dir/events.ndjson" >> "$combined" || rc=1
  fi
