...
# gluerun_ctx_rehydrate_sources deliberately does NOT resolve it; instead it must
...
# prints NOTHING otherwise. base_dir defaults to ${GLUERUN_ROOT:-.}. Its stdout is
# meant to be expanded as a trailing extra argument to gluerun_ctx_rehydrate_sources
# / gluerun_ctx_rehydrate_event_data at BOTH rehydrate sites, so the injected packet
...
# gluerun_ctx_artifact_exclude over every spec, so a single quarantine authority is
...
gluerun_ctx_rehydrate_decision_source() {
  local base_dir="${1-}"
  [[ -n "$base_dir" ]] || base_dir="${GLUERUN_ROOT:-.}"
