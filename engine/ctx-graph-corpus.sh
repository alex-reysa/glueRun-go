...
# (OFF-parity when SINGULAR_CTX_GRAPH is unset or 0). The writer touches only the
# explicitly-provided <graphdir> and no other path.
#
# This is the determinism/idempotence keystone every later graph-projector
# capability (singular graph rebuild/sync, behind SINGULAR_CTX_GRAPH, default 0)
# funnels through: it turns the stream of projected JSONL lines emitted by
# engine/ctx-graph.sh's singular_graph_emit_node / singular_graph_emit_edge into
...
#       idempotent.
#   singular_graph_write_corpus <graphdir> <nodesInput> <edgesInput>
#       Run each input stream through singular_graph_canonicalize and write
...
#       defaults to ${SINGULAR_CTX_GRAPH_DIR:-.singular-state/graph} when not
...
# streams, each run through singular_graph_canonicalize. Prior graph files are
...
singular_graph_write_corpus() {
  local graphdir="${1:-}" nodes_in="${2:-}" edges_in="${3:-}"
  [[ -n "$graphdir" ]] || graphdir="${SINGULAR_CTX_GRAPH_DIR:-.singular-state/graph}"
  ...
  singular_graph_canonicalize < "$nodes_in" > "$graphdir/nodes.jsonl" || return 1
  singular_graph_canonicalize < "$edges_in" > "$graphdir/edges.jsonl" || return 1
}
