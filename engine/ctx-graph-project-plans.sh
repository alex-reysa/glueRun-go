...
# (OFF-parity when GLUERUN_CTX_GRAPH is unset or 0). The mappers write NO file:
...
# gluerun_graph_node_id / gluerun_graph_emit_node / gluerun_graph_emit_edge —
# extending the graph-projector's inner layer with the plan-lifecycle node/edge
# families. A later `gluerun graph rebuild` entry point (behind GLUERUN_CTX_GRAPH,
# default 0) partitions the stream by `kind` and hands the node/edge streams to
# the integrated corpus writer gluerun_graph_write_corpus (engine/ctx-graph-corpus.sh).
#
...
#
# Public functions:
#   gluerun_graph_project_plan_versions <eventsPath>
...
#   gluerun_graph_project_decisions <eventsPath>
...
#   gluerun_graph_project_critique <planCritiqueRecordPath>
#       -> reads a gluerun.orchestration.plan-critique.v0 record (node, runId,
...
gluerun_graph_project_plan_versions() {
  local events_path="$1"
  ...
  while IFS=$'\037' read -r node run_id revises content_b64; do
    ...
    ident="$(gluerun_graph_identity plan-version "$node" "$run_id")"
    version_node="$(gluerun_graph_node_id "$ident")"
    gluerun_graph_emit_node plan-version "$ident" "$events_path" "$content"
    if [[ -n "$revises" ]]; then
      prior_ident="$(gluerun_graph_identity plan-version "$node" "$revises")"
      prior_node="$(gluerun_graph_node_id "$prior_ident")"
      gluerun_graph_emit_edge revises    "$version_node" "$prior_node" "$events_path" "$content"
      gluerun_graph_emit_edge supersedes "$version_node" "$prior_node" "$events_path" "$content"
    fi
  done < <(GLUERUN_GP_PATH="$events_path" python3 -c '
import json, os, base64
US = "\x1f"
for line in open(os.environ["GLUERUN_GP_PATH"], encoding="utf-8"):
    line = line.strip()
    if not line:
        continue
    ev = json.loads(line)
...
gluerun_graph_project_decisions() {
  local events_path="$1"
  ...
  while IFS=$'\037' read -r etype node run_id ts content_b64; do
    [[ -n "$content_b64" ]] || continue
    content="$(printf '%s' "$content_b64" | base64 --decode)"
    ident="$(gluerun_graph_identity decision "$etype" "$node" "$run_id" "$ts")"
    gluerun_graph_emit_node decision "$ident" "$events_path" "$content"
  done < <(GLUERUN_GP_PATH="$events_path" python3 -c '
import json, os, base64
US = "\x1f"
...
# attributes.severity projected) from a gluerun.orchestration.plan-critique.v0
# record. Deterministic and idempotent; pure stdout — writes no file.
gluerun_graph_project_critique() {
  local record_path="$1"
...
import json, os, base64
d = json.load(open(os.environ["GLUERUN_GP_PATH"]))
...
') || return 1
  ...
  local crit_ident
  crit_ident="$(gluerun_graph_identity critique "$node" "$run_id")"
  gluerun_graph_emit_node critique "$crit_ident" "$record_path" "$record_content"
  ...
  while IFS=$'\037' read -r fid severity content_b64; do
    [[ -n "$content_b64" ]] || continue
    content="$(printf '%s' "$content_b64" | base64 --decode)"
    ident="$(gluerun_graph_identity finding "$node" "$run_id" "$fid")"
    attrs="$(GLUERUN_GP_SEV="$severity" python3 -c '
import json, os
print(json.dumps({"severity": os.environ["GLUERUN_GP_SEV"]}, separators=(",", ":")))')"
    gluerun_graph_emit_node finding "$ident" "$record_path" "$content" "" "$attrs"
  done < <(GLUERUN_GP_PATH="$record_path" python3 -c '
import json, os, base64
US = "\x1f"
d = json.load(open(os.environ["GLUERUN_GP_PATH"]))
