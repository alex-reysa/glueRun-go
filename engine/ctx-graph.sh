...
# (OFF-parity when SINGULAR_CTX_GRAPH is unset or 0). Nothing here writes to the
...
# (singular graph rebuild/sync/query, behind SINGULAR_CTX_GRAPH, default 0)
...
#
# Public functions:
#   singular_graph_content_hash <content>
#       -> sha256:<64-hex> of the source record's canonical content.
#   singular_graph_node_id <identity>
#       -> n-<12-hex> stable id for a source-record identity string.
#   singular_graph_edge_id <fromId> <edgeType> <toId>
#       -> e-<12-hex> stable id for the directed identity triple.
#   singular_graph_emit_node <type> <identity> <sourcePath> <content> [label] [attributesJson]
...
#   singular_graph_emit_edge <edgeType> <fromId> <toId> <sourcePath> <content> [attributesJson]
...
_singular_graph_sha256() {
  printf '%s' "$1" | shasum -a 256 | awk '{print $1}'
}

singular_graph_content_hash() {
  printf 'sha256:%s' "$(_singular_graph_sha256 "$1")"
}

singular_graph_node_id() {
  local h
  h="$(_singular_graph_sha256 "$1")"
  printf 'n-%s' "${h:0:12}"
}

singular_graph_edge_id() {
  ...
  # direction-sensitive.
  local h
  h="$(_singular_graph_sha256 "$(printf '%s\037%s\037%s' "$1" "$2" "$3")")"
  printf 'e-%s' "${h:0:12}"
}

# --- Slices 2 & 3: pure-stdout JSONL emitters --------------------------------

singular_graph_emit_node() {
  ...
  SINGULAR_G_KIND="node" \
  SINGULAR_G_ID="$(singular_graph_node_id "$identity")" \
  SINGULAR_G_TYPE="$ntype" \
  SINGULAR_G_EV="$ev" \
  SINGULAR_G_SP="$source_path" \
  SINGULAR_G_CH="$(singular_graph_content_hash "$content")" \
  SINGULAR_G_LABEL="$label" \
  SINGULAR_G_ATTR="$attributes" \
  python3 -c '
import json, os
o = {
    "schema": "singular.orchestration.context-graph.v0",
    "kind": "node",
    "id": os.environ["SINGULAR_G_ID"],
    "type": os.environ["SINGULAR_G_TYPE"],
    "evidenceClass": os.environ["SINGULAR_G_EV"],
    "provenance": {
        "sourcePath": os.environ["SINGULAR_G_SP"],
        "contentHash": os.environ["SINGULAR_G_CH"],
    },
}
label = os.environ.get("SINGULAR_G_LABEL", "")
if label:
    o["label"] = label
attr = os.environ.get("SINGULAR_G_ATTR", "")
if attr:
...
singular_graph_emit_edge() {
  local etype="$1" from_id="$2" to_id="$3" source_path="$4" content="$5"
  local attributes="${6:-}"
  SINGULAR_G_ID="$(singular_graph_edge_id "$from_id" "$etype" "$to_id")" \
  SINGULAR_G_TYPE="$etype" \
  SINGULAR_G_FROM="$from_id" \
  SINGULAR_G_TO="$to_id" \
  SINGULAR_G_SP="$source_path" \
  SINGULAR_G_CH="$(singular_graph_content_hash "$content")" \
  SINGULAR_G_ATTR="$attributes" \
...
o = {
    ...
    "id": os.environ["SINGULAR_G_ID"],
    "type": os.environ["SINGULAR_G_TYPE"],
    "from": os.environ["SINGULAR_G_FROM"],
    "to": os.environ["SINGULAR_G_TO"],
    "provenance": {
        "sourcePath": os.environ["SINGULAR_G_SP"],
        "contentHash": os.environ["SINGULAR_G_CH"],
    },
