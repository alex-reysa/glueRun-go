...
| audit verdict | `.singular-state/runs/<RUN>/audit.json` | `taskId` | `verdict` (`accepted/needs-fix/blocked/needs-human`) |
| decider verdict | `.singular-state/runs/<RUN>/decision-<class>.json` | `taskId` | `action` (`retry/…/escalate-parked`) + `failureClass` |
| state packet | `.singular-state/inbox/<RUN>.json`, `docs/orchestration/packets/imported/<TASK>/` | `taskId` | `status` (`needs-review/accepted`) |
| L2 lease | `.singular-state/leases/<TASK>.json` | `taskId` | `status` (`planned/running/needs-review/accepted/integrated/failed/blocked/cancelled/superseded/stale`) |
| L1 lease | `.singular-state/l1-leases/<node>.json` | `node` | `status` (`proposed/planning/active/released/failed`) |
| dispatch record | `.singular-state/dispatch/<TASK>.json` (+`.exit`) | `taskId` | `state` (`launched/reaped`) + `outcome` (`ok/refused/terminal/failed/crashed`) |
| events journal | `.singular-state/events.ndjson` (**ndjson, not .jsonl/.log**) | `type` | per-type `data{}` |
...
| planner backoff | `.singular-state/planner-backoff.json` | — | `failureClass` + `until` + `logRef` (never empty for quota since 0.5.0) |
| breaker | `.singular-state/circuit.json` | — | `consecFails` |
| origin state | `.singular-state/origin-state.json` | — | `gates{passed,total}`, `completedNodes`, `taskCounts` (0.5.0) |
...
Prefer `singular health --json` / `singular gates --json` over hand-rolled
...
jq -r 'select(.type=="decider.verdict")
  ...
  .singular-state/events.ndjson | tail -20
...
singular next-areas --explain | jq '{frontier: [.frontier[].node], excluded}'
```
